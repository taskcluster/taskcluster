...
/**
 * This file contains functions specific to querying and working with
 * Taskcluster APIs
...
export async function fetchTaskGroup(server, taskGroupId, updateStatusMessage) {
  ...
  if (!response.ok) {
    ...
    return Promise.reject(new Error('Could not fetch task.'));
  }
  ...
  for (let page = 0; page < maxPages; page += 1) {
    ...
    const continuationUrl = `${listUrl}?continuationToken=${nextTaskGroup.continuationToken}`;
...
/**
 * TODO - This should probably be removed from the official
 * Taskcluster implementation.
 *
...
/**
 * TODO - This should probably be removed from the official Taskcluster
 * implementation.
 *
...
/**
 * TODO - Determine if fetchDependentTasks should be retained and surfaced in
 * the new Taskcluster UI
 *
...
export async function getTasks(
  ...
  if (taskGroupIds.length && taskGroupIds.some(id => !isTaskGroupIdValid(id))) {
    ...
    p.innerText = `A task group id was not valid, ${JSON.stringify(
      taskGroupIds
    )}`;
  ...
  for (const taskId of dependencies) {
    ...
    try {
      ...
      for (const { task, status } of taskGroup.tasks) {
        ...
        for (const id of task.dependencies) {
          // Add on the to dependencies. The iterator will continue iterating
          // on all of the newly discovered dependencies.
          dependencies.add(id);
        }
      }
