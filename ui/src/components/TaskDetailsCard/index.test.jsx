...
it('should sort the dependents by name', () => {
  const dependent = (taskId, name) => ({
    task: { taskGroupId: 'tg1', metadata: { name } },
    status: { taskId, state: 'completed', runs: [] },
  });
  const { getAllByText, getByText } = renderCard({
    dependents: [dependent('b', 'bravo'), dependent('a', 'alpha')],
  });
  const names = () => {
    return getAllByText(/^(alpha|bravo)$/).map(el => el.textContent);
  };
