...
describe('summarizeAuthorization', () => {
  it('should return empty values', () => {
    ...
    expect(out[0].value).toEqual('0 / 0');
    expect(out[0].error).toBeUndefined();
    expect(out[0].link).toEqual('/auth/clients');
    expect(out[1].link).toEqual('/auth/roles');
    expect(out[2].link).toEqual('/secrets');
  });
  ...
  it('should return counts', () => {
    ...
    expect(out.length).toEqual(3);
    expect(out[0].value).toEqual('2 / 1');
    expect(out[1].value).toEqual('2');
