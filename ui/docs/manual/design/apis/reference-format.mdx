...
substituted.  The request URL is formed from the Taskcluster [root URL](/docs/manual/using/root-urls), the
service name, the API version, and the post-substitution route, as described in the [URL Structure reference](/docs/reference/url-structure).
...
Each service generates metrics that match formats defined in metrics references.
These reference documents have metadata name `metrics` and the following schema:

<SchemaTable schema="/schemas/common/metrics-reference-v0.json#" />
