...
`TASK_ID` is set in the environment of tasks running on generic-worker (whether
they use the generic-worker or the legacy docker-worker payload format) to the
`taskId` of the current task.

## RUN_ID

`RUN_ID` is set in the environment of tasks running on generic-worker (whether
they use the generic-worker or the legacy docker-worker payload format) to the
`runId` of the current task run.

## TASK_GROUP_ID

`TASK_GROUP_ID` is set in the environment of tasks running on generic-worker
(whether they use the generic-worker or the legacy docker-worker payload
format; in the latter case d2g forwards it into the container) to the
`taskGroupId` of the current task.
