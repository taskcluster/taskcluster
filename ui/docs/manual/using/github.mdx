...
(draft)
- running tasks on commit pushes and pull requests
- re-running failed tasks from github ui
- creating check-runs with extended information
- updating build statuses on commits and pull requests
- policy check if contributor or not
- allow comments in prs to trigger tasks
- run tasks on release event
