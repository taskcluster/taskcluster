...
> **Note:** Examples below use the docker-worker payload format. These tasks now run on generic-worker, which accepts this legacy payload format on Linux through the [d2g](https://github.com/taskcluster/taskcluster/tree/main/tools/d2g) translation layer for backward compatibility.
...
enables the taskclusterProxy feature:
...
The role for the `main` branch of the repository,
`github.com/softwarez/myapp:branch:main`, contains the same scope
`secrets:get:repo:github.com/softwarez/myapp`

Note that the `..:pull-request` role does *not* have this scope, so pull
requests cannot upload documentation.

See [Calling Taskcluster APIs](/docs/manual/using/api) for more on using API methods within tasks.
