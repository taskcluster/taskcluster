...
run every 15 minutes. Its task body contains a docker-worker-format payload (run by generic-worker via [d2g](https://github.com/taskcluster/taskcluster/tree/main/tools/d2g)) that runs
the same script as for a push to the `main` branch, and provides the necessary
scopes.
...
be creating more tasks (for example, a [decision task](/docs/manual/design/conventions/decision-task)).
...
For more complex purposes, invoke a [decision task](/docs/manual/design/conventions/decision-task)
