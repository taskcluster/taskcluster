...
in tasks using the generic-worker payload format via the `logs.live` property,
or in tasks using the docker-worker payload format via the `log` property.
...
[Generic-Worker](/docs/reference/workers/generic-worker), that looks like
...
For tasks using the legacy Docker-Worker payload format (translated by [d2g](https://github.com/taskcluster/taskcluster/tree/main/tools/d2g)), the format is
slightly different:
...
In either case generic-worker will upload all files in the `workspace/results`
directory as artifacts named `public/results/<filename>`. Note that the
docker-worker payload format requires an absolute path, while the generic-worker
payload format requires a relative path.

Both payload formats support a `path` that points to a specific file. However,
a task will fail if this file does not exist.
