...
Create a new resource group, which will be used as the default destination for the worker resources this provider creates (note that the location of the resource group is not related to the location of the workers).
Launch configurations may also opt into per-pool resource groups via `armDeploymentResourceGroup`; see [Resource Group Strategy](/docs/reference/core/worker-manager/azure#resource-group-strategy) for the trade-offs and the recommended Azure Policy pairing.
...
The example custom role below is the **base set of `Actions`** that the worker-manager itself exercises — i.e. the permissions required regardless of which launch configurations you define.
A worker pool's launch configuration may reference additional resources via its ARM template (e.g. availability sets, scale sets, custom NSG rules, boot-diagnostics storage accounts, or any other Azure resource type), and the role will need the corresponding `Actions` added on top before that pool can provision.
The full list of available actions per resource provider is at [Azure resource provider operations](https://learn.microsoft.com/en-us/azure/role-based-access-control/resource-provider-operations).

Add the role under your subscription -> "IAM" -> "Create a Custom Role", and start from JSON using content like this:

```json
{
  "Name": "Worker-Manager Azure Provider",
  "IsCustom": true,
  "Description": "Minimum permissions for the Taskcluster worker-manager Azure provider.",
  "Actions": [
...
If your launch configurations enable boot diagnostics with a custom storage account, also add `Microsoft.Storage/storageAccounts/read` and `Microsoft.Storage/storageAccounts/listKeys/action`.

If a launch configuration deploys from a [template spec](https://learn.microsoft.com/en-us/azure/azure-resource-manager/templates/template-specs) (i.e. `armDeployment.templateLink.id`), also add `Microsoft.Resources/templateSpecs/read` and `Microsoft.Resources/templateSpecs/versions/read` so the service principal can read the spec at deploy time.

<Warning>
If any launch configuration uses `armDeploymentResourceGroup` to deploy into a per-pool resource group, the role must additionally include `Microsoft.Resources/subscriptions/resourceGroups/write` so the worker-manager service principal can create that resource group. See [Resource Group Strategy](/docs/reference/core/worker-manager/azure#resource-group-strategy) in the Azure provider reference for the trade-offs and the recommended Azure Policy pairing that bounds which resource-group names this grant can actually create.
</Warning>
