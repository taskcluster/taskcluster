...
* Under `Subscribe to events` check the following boxes:
  ...
  * Push
  * Release
  * Check run (if you want to receive "rerun" requests from GitHub UI for failed checks)
  * Issue comment (to allow contributors to trigger tasks by commenting on pull requests)
