...
To enable this support, the environment variable `NEW_RELIC_ENABLED` must be set to `true`.
All other configuration must be supplied via environment variables.
If this environment variable is not set to `true`, then the new relic module will not be loaded at startup -- if you are not using New Relic, you pay no price.
