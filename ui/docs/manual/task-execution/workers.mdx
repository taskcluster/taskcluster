...
The Taskcluster-provided worker implementation is described in [generic-worker](/docs/reference/workers/generic-worker). Generic-worker also accepts the legacy docker-worker payload format on Linux, translated transparently by [d2g](https://github.com/taskcluster/taskcluster/tree/main/tools/d2g). Tasks written against the docker-worker payload format continue to run unchanged.
