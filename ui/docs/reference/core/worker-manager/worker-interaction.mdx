...
supplying it with a one time usage `secret`. The secret value will have been either last configured
in `registerWorker` (in the case of a newly registerd worker) or `reregisterWorker`.
The expiration time of the new credentials can be controlled via `reregistrationTimeout` which gets set when
calling [`workerManager.registerWorker`](/docs/reference/core/worker-manager/api#registerWorker).
