...
if worker process is being terminated with out of memory error, it is not possible to catch it and report it to the worker-manager.
Also, when image is not configured properly, worker might be able to start and register, but would fail to call `queue.claimWork`.
