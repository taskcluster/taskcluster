...
This allows a task to run without seccomp, similar to running docker with `--security-opt seccomp=unconfined`.
This only works for worker-types configured to enable it.
...
Requires `docker-worker:capability:disableSeccomp` scope.
