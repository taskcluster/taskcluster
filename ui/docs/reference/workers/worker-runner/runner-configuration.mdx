...
The `cacheOverRestarts` option is set because generic-worker will reboot after every task (unless `headlessTasks` is enabled), so worker-runner must cache its state over those reboots.
