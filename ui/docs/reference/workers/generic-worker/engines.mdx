...
Compatibility with the legacy docker-worker payload format is provided on Linux
by the [d2g](https://github.com/taskcluster/taskcluster/tree/main/tools/d2g)
translation layer inside generic-worker, which converts docker-worker payloads
into equivalent generic-worker payloads that invoke docker/podman commands
directly.
