...
concrete requirements. In particular, they contain outdated references to
the "legacy" deployment of Taskcluster at https://taskcluster.net.
...
Run `go get -tags multiuser github.com/taskcluster/taskcluster/v<MAJOR>/workers/generic-worker` (windows/darwin) and/or `go get -tags simple github.com/taskcluster/taskcluster/v<MAJOR>/workers/generic-worker` (linux/darwin). This should also build binaries for your platform.
...
1. Ensure that the Generic Worker binary is readable and executable by _all_
   users, _not_ only `LocalSystem`. Failure to do this will cause exit code 69
   (internal error).

1. Run the following command to generate an ed25519 private key for signing
   artifacts:
...
1. Download NSSM 2.24 from https://nssm.cc/release/nssm-2.24.zip and extract it
   under `C:\`.

1. Install generic-worker as a Windows service running under the `LocalSystem`
...
1. Download livelog from https://github.com/taskcluster/taskcluster/releases/ and
   place it in `C:\generic-worker\livelog.exe`.

1. Download taskcluster proxy from
   https://github.com/taskcluster/taskcluster/releases/ and place it in
   `C:\generic-worker\taskcluster-proxy.exe`.

1. Create `C:\generic-worker\generic-worker.config` with appopriate values.

1. Edit file `C:\generic-worker\generic-worker.config` with appropriate
   settings (see `generic-worker.exe --help` for information).

1. Reboot the machine, and the worker should be running. Check logs under
...
__These instructions require macOS Mojave version 10.14.x__
 ...
 1. Download `generic-worker` from
    https://github.com/taskcluster/taskcluster/releases/ and place it in
 ...
 1. Make `generic-worker`, `taskcluster-proxy`, `livelog` binaries executable by _all_ users:

    * `chmod a+x /usr/local/bin/{generic-worker,taskcluster-proxy,livelog}`

 1. Generate a key for signing artifacts:
 ...
 1. Create the file `/usr/local/bin/run-generic-worker.sh` with the following content:
 ...
 1. Run the following to make the `run-generic-worker.sh` script executable:

    * `chmod a+x /usr/local/bin/run-generic-worker.sh`

 1. Create `/etc/generic-worker/config` with appropriate configuration settings
 ...
 1. Watch for logs in `/var/log/generic-worker/`.


### Linux simple/multiuser build
 ...
 1. Install curl (needed for later). For example, on ubuntu:

    ```
    apt install -y curl
    ```

 1. For _multiuser_ ensure Gnome Desktop 3 is installed:

    ```
    apt install -y ubuntu-desktop ubuntu-gnome-desktop
    ```

 1. If tasks require Docker, install it. For example,
    on ubuntu bionic:
 ...
 1. Download `generic-worker`, `taskcluster-proxy` and `livelog` to `/usr/local/bin`:
 ...
 1. Create directories required by generic-worker:

    ```
    mkdir -p /etc/generic-worker /var/local/generic-worker
    ```

 1. Check generic-worker works and has correct version:
 ...
 1. Ensure host 'taskcluster' resolves to localhost:

    ```
    echo 127.0.1.1 taskcluster >> /etc/hosts
    ```

 1. __AWS workers only__: configure generic-worker to run on boot with
 ...
 1. __Non-AWS workers only__: configure generic-worker to run on boot with
