...
Please note that only the user or organization authoring the request is being checked, even if the event itself comes from another user (for example, if the PR was opened by a non-collaborator, or from a fork in a non-collaborator organization, closing the PR or editing it by a collaborator will not result in a build in Taskcluster).
