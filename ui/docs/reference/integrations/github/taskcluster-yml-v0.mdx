...
You can also modify a task definition so that it will only run for events on certain branches. For example, the task defined below will only run for pushes to the `main` branch:
...
You can even choose to exclude certain branches from your task run. The task will run only if it is not on one of the excluded branches. For example, the task defined below will only run for pushes **not** to the `main` branch:
...
```
  ...
  GITHUB_BRANCH          | "{{ event.head.repo.branch }}" | main
                         |                                |
  ...
  GITHUB_BASE_BRANCH     | "{{ event.base.repo.branch }}" | main
  GITHUB_BASE_REF        | "{{ event.base.ref }}"         | refs/heads/main
                         |                                |
...
```
  ...
  GITHUB_BRANCH          | "{{ event.base.repo.branch }}" | main
                         |                                |
  GITHUB_HEAD_USER       | "{{ event.head.user.login }}"  | maryscott
  GITHUB_HEAD_REPO_NAME  | "{{ event.head.repo.name }}"   | somerepo
  GITHUB_HEAD_REPO_URL   | "{{ event.head.repo.url }}"    | https://github.com/maryscott/somerepo
