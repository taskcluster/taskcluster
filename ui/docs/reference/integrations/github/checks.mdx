...
in the check run. To do so, it looks for an artifact on the task named `public/github/customCheckRunAnnotations.json` and uses the artifact content as the check run text.
