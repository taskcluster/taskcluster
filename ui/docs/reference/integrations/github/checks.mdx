...
If you use a non-default artifact path, your Taskcluster deployment must include `queue:get-artifact:<artifactPath>` in
the `static/taskcluster/github` client's static scopes.
...
If you use a non-default artifact path, your Taskcluster deployment must include `queue:get-artifact:<artifactPath>` in the
`static/taskcluster/github` client's static scopes. The json in that file will be passed along unchanged to Github and should be in
their annotation format defined in the link above.
...
* If you see a `Cannot GET blahblah (403)` kind of error, it's likely that your artifact is private. Either use
  a public artifact (see note on the naming convention above) or ensure the `static/taskcluster/github` client
  has the `queue:get-artifact:<artifactPath>` scope.
* If there is no error at all and you don't see your custom text in the checkrun, it means there was a
  ...
  artifact or in your `extra.github.customCheckRun` section if you use one. Make sure the artifact exists and is
  named or referenced correctly.
