...
Actions were defined at the provisioner level via the queue's
`declareProvisioner` method. That method has been removed, so it is no longer
possible to define new actions.
