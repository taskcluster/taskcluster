...
For most changes, the author makes a pull request using the usual GitHub process.
If there is an issue or Bugzilla bug to which it relates, the description for the pull request should point to it with a link.
It's also OK to make a pull request to fix something without first filing an issue.

We have configured GitHub to automatically assign reviewers in most cases.
We typically review changes within a few working days.
Feel free to comment if you think your pull request has been missed.
