...
Let's try to run whole Taskcluster locally to see what it consists of and
how services interact with each other and.

You would need `git` to clone [taskcluster/taskcluster](https://github.com/taskcluster/taskcluster) repository:
...
To start everything simply run `docker compose up -d`.

This should start following services:
...
* `taskcluster` which is an nginx reverse-proxy that serves all of Taskcluster services through single endpoint
...
You might also want to add custom domain entry to your `/etc/hosts` file to be sure that UI can work properly with tasks and artifacts:
...
From now on Taskcluster will know that you are logged in and will allow you to do perform various actions.
...
* `docker-compose` will have a single worker called `generic-worker`. This is a [simple generic worker](/docs/reference/workers/generic-worker) that can run real commands.
...
To make sure that timestamps are valid you can click on the `...` menu at the bottom and press `Update Timestamps` button.

Now it is fine to click `Save` button at the bottom at the page.

UI should redirect you to the newly create task page, i.e. <http://taskcluster/tasks/QiGSm7nTTkOVZKLYBPKjMA>.

You should be able to see that this task is `Running` now.

Once it will be finished, you could click on the `public/logs/live.log` artifact to see task execution log.
...
To stop everything simply do `docker compose down` to stop services and clean up resources.
