...
<SiteSpecific requires={['root_url']}>
...
If possible, choose a Linux generic-worker pool, since the example below uses the legacy docker-worker payload format that is supported on Linux generic-worker via the [d2g](https://github.com/taskcluster/taskcluster/tree/main/tools/d2g) translation layer.

<SiteSpecific requires={['tutorial_worker_pool_id']}>
In this deployment, `%tutorial_worker_pool_id%` is a good choice.
</SiteSpecific>

## Creating a Task

> **Note:** The example below uses the docker-worker payload format. This task now runs on generic-worker, which accepts this legacy payload format on Linux through the [d2g](https://github.com/taskcluster/taskcluster/tree/main/tools/d2g) translation layer for backward compatibility.
 ...
 * The `payload` uses the legacy docker-worker payload format, which generic-worker accepts on Linux via [d2g](https://github.com/taskcluster/taskcluster/tree/main/tools/d2g).
