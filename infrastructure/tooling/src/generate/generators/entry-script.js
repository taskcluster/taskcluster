...
import { listServices, readRepoFile, readRepoYAML, writeRepoFile } from '../../utils/index.js';
...
export const tasks = [
  {
    ...
    run: async (_requirements, _utils) => {
      ...
      procs['ui/web'] =
        'node /app/ui/generate-env-js.js /app/ui/build/static/env.js && ' +
        "exec nginx -c /app/ui/web-ui-nginx-site.conf -g 'daemon off;'";

      const entrypointScript = []
        .concat(['#!/bin/bash', '', '# DO NOT EDIT. This is an autogenerated file.', '', 'case "${1}" in'])
        .concat(Object.entries(procs).map(([process, command]) => `${process}) ${command};;`))
        .concat([
