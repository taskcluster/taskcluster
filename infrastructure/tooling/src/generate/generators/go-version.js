...
export const tasks = [
  {
    ...
    run: async (_requirements, utils) => {
      ...
      await modifyRepoFile('dev-docs/development-process.md', (contents) =>
        contents.replace(/Go version go[0-9.]+/, `Go version ${goVersion}`),
      );

      utils.status({ message: 'dev-docs/node-and-go-upgrades.md' });
      await modifyRepoFile('dev-docs/node-and-go-upgrades.md', (contents) =>
        contents.replace(/install go[0-9.]+/, `install ${goVersion}`).replace(/use go[0-9.]+/, `use ${goVersion}`),
      );

      utils.status({ message: 'go.mod' });
      await modifyRepoFile('go.mod', (contents) =>
        contents.replace(/^go [0-9.]+$/m, `go ${goVersionMajor}.${goVersionMinor}.${goVersionBugfix}`),
      );

      utils.status({ message: 'workers/generic-worker/build.sh' });
      await modifyRepoFile('workers/generic-worker/build.sh', (contents) =>
        contents
          .replace(/go [0-9.]+ or higher/g, `go ${goVersionMajor}.${goVersionMinor} or higher`)
          .replace(/GO_MAJOR_VERSION=[0-9]+/, `GO_MAJOR_VERSION=${goVersionMajor}`)
          .replace(/MIN_GO_MINOR_VERSION=[0-9]+/, `MIN_GO_MINOR_VERSION=${goVersionMinor}`),
      );

      ['generic-worker.Dockerfile', 'taskcluster/docker/ci/Dockerfile'].forEach(async (file) => {
