    ...
    const serviceEnv = (name) => {
      ...
      return Object.fromEntries(config.map(cfg => {
        ...
        switch (cfg.var) {
          ...
          case 'TASKCLUSTER_ROOT_URL':
            // ui needs root url to be accessible through browser, not inside docker compose network
            if (name === 'ui') {
              value = `http://localhost:${servicePrimaryPort('ingress')}`;
            }
            break;
      ...
      services: {
        ...
        'generic-worker': serviceDefinition('generic-worker', {
          ...
          depends_on: {
            ...
            ingress: { condition: 'service_started' },
          },
        }),
      },
    };
...
exports.tasks.push({
  title: `Generate docker-compose.dev.yml`,
  requires: [
    ...SERVICES.map(name => `configs-${name}`),
    ...SERVICES.map(name => `procslist-${name}`),
    'target-nginx.conf',
  ...
  run: async (requirements, utils) => {
    ...
    await writeRepoYAML(path.join('.', DEV_COMPOSE_FILENAME), dockerCompose);
    return {
      'target-docker-compose.dev.yml': dockerCompose,
    };
  },
});

exports.tasks.push({
  title: `Generate docker-compose.prod.yml`,
  requires: [
    ...SERVICES.map(name => `configs-${name}`),
    ...SERVICES.map(name => `procslist-${name}`),
    'target-nginx.conf',
  ],
  provides: ['target-docker-compose.prod.yml'],
  run: async (requirements, utils) => {
    const prodConfig = {
      environment: {
        NODE_ENV: 'production',
    ...
    for (let name of SERVICES) {
      ...
      Object.keys(procs).forEach((proc) => {
        ...
        dockerCompose.services[`${name}-${proc}`] = prodConfig;
      });
    }

    await writeRepoYAML(path.join('.', PROD_COMPOSE_FILENAME), dockerCompose);
    return {
      'target-docker-compose.prod.yml': dockerCompose,
    };
  },
});
