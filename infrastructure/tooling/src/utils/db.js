...
export const updateVersionsReadme = async (schema, releases) => {
  ...
  for (const version of schema.versions) {
    ...
    table.push(
      `| [${zpad}](./${zpad}.yml) | ${tcversion(releases, version.version) || '(pending release)'} | ${version.description || ''} |`,
    );
  }

  const versionsReadme = path.join('db', 'versions', 'README.md');
  await modifyRepoFile(versionsReadme, (content) =>
    content.replace(
...
export const updateDbFns = async (schema, releases, currentTcVersion) => {
  ...
  serviceNames.forEach((sn) => {
    ...
    services.set(
      sn,
      serviceMethods.sort((a, b) => a.name.localeCompare(b.name)),
    );
  });
  ...
  for (const [serviceName, methods] of services.entries()) {
    output.push(` * [${serviceName} functions](#${serviceName})`);
    for (const { name } of methods.filter((method) => !method.deprecated)) {
  ...
  for (const [serviceName, methods] of services.entries()) {
    ...
    for (const method of methods.filter((method) => !method.deprecated)) {
      ...
      if (tableReturns) {
        ...
        for (const r of tableReturns[1].split(', ')) {
          output.push(`  * \`${r}\``);
        }
      } else {
    ...
    for (const method of depMethods) {
      let version;
    ...
    for (const [serviceName, methods] of services.entries()) {
      ...
      for (const method of methods) {
        if ((isDeprecated && method.deprecated) || (!isDeprecated && !method.deprecated)) {
          output.push(
            `  ${method.name}: ${typeName(serviceName, method.name, method.deprecated ? 'DeprecatedFn' : 'Fn')};`,
          );
        }
      }
    }
