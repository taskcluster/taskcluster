...
program
  ...
  .option(
    '--docker-repo-generic-worker <generic-worker-docker-repo>',
    'docker repository to which generic worker image should be pushed'
  )
  ...
  .option(
    '--target <target>',
    'The thing to build, such as `monoimage` or `generic-worker` or `all`; default is monoimage'
  )
  .action(
    actFn(async ({ options }) => {
...
program
  ...
  .option('--logs-dir <logs-dir>', 'A directory to put debug logs. default <base-dir>/logs')
  .on('--help', () => {
    console.log(['', 'This command expects to run in automation in response to a push to CI.'].join('\n'));
  })
  .action(
    actFn(async ({ options }) => {
...
program
  .command('upgrade:node:prepare-shell')
...
program
  .command('upgrade:go:prepare-shell')
...
program.command('test:meta').action(
  actFn(async ({ options }) => {
...
program.command('*', { noHelp: true }).action(() => program.help(txt => txt));
