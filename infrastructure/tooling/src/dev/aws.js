...
const setupIam = async ({ iam = new IAMClient(), iamName, iamPolicy }) => {
  ...
  const { AccessKeyMetadata: existingKeys } = await iam.send(
  ...
  const { AccessKey: accessKey } = await iam.send(
...
export default async ({ userConfig, answer, configTmpl }) => {
  ...
  if (
    !userConfig.meta.lastAppliedPublicBucketPolicy ||
        ...
        Statement: [
          {
            ...
            Resource: [`arn:aws:s3:::${publicBucketName}/*`, `arn:aws:s3:::${privateBucketName}/*`],
          },
          {
            Effect: 'Allow',
            Action: [
