...
The build process produces a Taskcluster release, which takes the form of a [Terraform JSON](./terraform-json.mdx) file.
...
The build process takes a [cluster spec](./cluster-spec.mdx) and source code and, along with lots of external resources like docker images, third-party packages, and so on, produces a "release".
...
The output of the build process is a Taskcluster release, which takes the form of a [Terraform JSON](./terraform-json.mdx) file.
