...
# now capture the AMI
IMAGE_ID="$(aws --region us-west-2 ec2 create-image --instance-id "${INSTANCE_ID}" --name "win2012r2 mozillabuild pmoore" --description "firefox desktop builds on windows - taskcluster worker" | sed -n 's/^ *"ImageId": *"\(.*\)" *$/\1/p')"
