...
function install {
  if [ "${1}" != 'native' ]; then
    ...
    GOOS="${1}" GOARCH="${2}" go test -c github.com/taskcluster/taskcluster/v62/tools/taskcluster-proxy
  else
    go get -ldflags "-X main.revision=$(git rev-parse HEAD)" -v ./...
    go vet ./...
    # note, this just builds tests, it doesn't run them!
    go test -c github.com/taskcluster/taskcluster/v62/tools/taskcluster-proxy
  fi
}
