...
####################################################################
  CGO_ENABLED=1 GORACE="history_size=7" go test -failfast -ldflags "-X github.com/taskcluster/taskcluster/v97/workers/generic-worker.revision=${GIT_REVISION}" -race -timeout 1h ./...
