...
# when a new major release is made.
####################################################################
  CGO_ENABLED=1 GORACE="history_size=7" go test -tags simple -failfast -ldflags "-X github.com/taskcluster/taskcluster/v44/workers/generic-worker.revision=$(git rev-parse HEAD)" -race -timeout 1h ./...
...
# when a new major release is made.
####################################################################
    CGO_ENABLED=1 GORACE="history_size=7" go test -tags docker -failfast -ldflags "-X github.com/taskcluster/taskcluster/v44/workers/generic-worker.revision=$(git rev-parse HEAD)" -race -timeout 1h ./...
  fi
