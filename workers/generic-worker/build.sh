...
####################################################################
  CGO_ENABLED=1 GORACE="history_size=7" go test -tags simple -failfast -ldflags "-X github.com/taskcluster/taskcluster/v56/workers/generic-worker.revision=$(git rev-parse HEAD)" -race -timeout 1h ./...
