...
import (
	...
	"github.com/taskcluster/taskcluster/v83/workers/generic-worker/errorreport"
	"github.com/taskcluster/taskcluster/v83/workers/generic-worker/expose"
	"github.com/taskcluster/taskcluster/v83/workers/generic-worker/fileutil"
	"github.com/taskcluster/taskcluster/v83/workers/generic-worker/graceful"
...
func (task *TaskRun) ExecuteCommand(index int) *CommandExecutionError {
	...
	go c.MonitorResources(func() bool {
		task.Warn("Sustained memory usage above 90%!")
		if config.DisableOOMProtection {
			task.Warn("DisableOOMProtection worker config is disabled, not aborting task...")
			return false
		} else {
