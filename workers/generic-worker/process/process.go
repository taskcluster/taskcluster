...
// If memory usage exceeds maxSystemMemoryPercent and available memory is less than
// minAvailableMemoryBytes for longer than highMemoryTimeoutSecs seconds,
		...
		for {
			...
			case <-ticker.C:
				if vm, err := mem.VirtualMemory(); err == nil {
					...
					// if memory used is greater than maxSystemMemoryPercent
					...
					if vm.UsedPercent > float64(maxSystemMemoryPercent) && vm.Available < minAvailableMemoryBytes {
