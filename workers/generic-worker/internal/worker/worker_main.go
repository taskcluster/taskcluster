...
import (
	...
	"github.com/taskcluster/taskcluster/v90/internal"
	"github.com/taskcluster/taskcluster/v90/internal/mocktc/tc"
	"github.com/taskcluster/taskcluster/v90/internal/scopes"
	"github.com/taskcluster/taskcluster/v90/workers/generic-worker/artifacts"
	"github.com/taskcluster/taskcluster/v90/workers/generic-worker/errorreport"
	"github.com/taskcluster/taskcluster/v90/workers/generic-worker/expose"
	...
	"github.com/taskcluster/taskcluster/v90/workers/generic-worker/metrics"
	"github.com/taskcluster/taskcluster/v90/workers/generic-worker/process"
	"github.com/taskcluster/taskcluster/v90/workers/generic-worker/protocol"
...
// logEvent is a helper function that logs worker metrics events
func logEvent(eventType string, task *TaskRun, timestamp time.Time) {
	workerConfig := metrics.WorkerConfig{
	...
	var taskInfo *metrics.TaskInfo
	if task != nil {
		taskInfo = &metrics.TaskInfo{
			TaskID: task.TaskID,
			RunID:  task.RunID,
		}
	}

	metrics.LogEvent(eventType, workerConfig, taskInfo, timestamp)
}
...
// Main is the entry point into the generic worker, exported for use by the main package
func Main() {
	...
	case arguments["run"]:
		...
		protocol.Initialize(os.Stdin, os.Stdout, withWorkerRunner, func(creds *tcclient.Credentials) {
...
func HandleCrash(r any) {
	...
	if protocol.Protocol != nil {
		errorreport.Send(protocol.Protocol, r, debugInfo)
	}
...
func exitOnError(exitCode ExitCode, err error, logMessage string, args ...any) {
	...
	if protocol.Protocol != nil {
		errorreport.Send(protocol.Protocol, combinedErr, debugInfo)
	}
	os.Exit(int(exitCode))
}
