...
import (
	...
	"github.com/taskcluster/taskcluster/v99/clients/client-go/tcsecrets"
...
func TestNoWorkerTypeUserDataGenericWorkerProperty(t *testing.T) {
	test := GWTest(t)
	_ = test.MockEC2(map[string]any{"foo": "bar"})
	err := test.Setup()
	defer test.Teardown()
	ExpectError(t, "no genericWorker object defined in worker pool definition and no readable generic worker config file", err)
}

func TestNoPublicConfig(t *testing.T) {
	test := GWTest(t)
	_ = test.MockEC2(map[string]any{
		"genericWorker": map[string]any{
			"files": []fileutil.File{},
		},
	})
	err := test.Setup()
	defer test.Teardown()
	ExpectError(t, "no genericWorker.config object defined in worker pool definition and no readable generic worker config file", err)
}
...
func TestNoWorkerTypeSecret(t *testing.T) {
	test := GWTest(t)
	_ = test.MockEC2(nil) // fetch config from worker manager / taskcluster secrets
	err := test.Setup()
	defer test.Teardown()
	ExpectNoError(t, err)
}
...
func TestAdditionalPropertyUnderGenericWorker(t *testing.T) {
	test := GWTest(t)
	_ = test.MockEC2(map[string]any{
		"genericWorker": map[string]any{
			"whoops": 123,
...
func TestNoShutdown(t *testing.T) {
	test := GWTest(t)
	ec2 := test.MockEC2(nil) // fetch config from worker manager / taskcluster secrets
...
func TestAWSWorkerTypeMetadata(t *testing.T) {
			...
			"config": map[string]any{
				...
				"workerTypeMetadata": map[string]any{
					"machine-setup": map[string]string{
						"pretend-metadata": "My value was configured in worker manager",
					},
				},
			},
		},
	...
	machineSetup := md["machine-setup"].(map[string]any)
...
func TestDeploymentIDUpdated(t *testing.T) {
	test := GWTest(t)
	test.Config.DeploymentID = "newDeploymentID"
	_ = test.MockEC2(nil) // fetch config from worker manager / taskcluster secrets
	err := test.Setup()
	defer test.Teardown()
	ExpectNoError(t, err)
...
func TestDeploymentIDNotUpdated(t *testing.T) {
	test := GWTest(t)
	test.Config.DeploymentID = "oldDeploymentID"
	_ = test.MockEC2(nil) // fetch config from worker manager / taskcluster secrets
	err := test.Setup()
	defer test.Teardown()
	ExpectNoError(t, err)
