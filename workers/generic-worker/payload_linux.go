...
import (
	...
	"github.com/taskcluster/taskcluster/v75/tools/jsonschema2go/text"
...
func (task *TaskRun) convertDockerWorkerPayload() *CommandExecutionError {
	...
	task.Definition.Scopes, err = d2g.ConvertScopes(task.Definition.Scopes, dwPayload, taskQueueID, config.PublicPlatformConfig.D2GConfigContainerEngine(), serviceFactory.Auth(config.Credentials(), config.RootURL))
	if err != nil {
		return MalformedPayloadError(err)
	}

	// Convert dwPayload to gwPayload
	gwPayload, err := d2g.ConvertPayload(dwPayload, config.PublicPlatformConfig.D2GConfigContainerEngine())
	if err != nil {
