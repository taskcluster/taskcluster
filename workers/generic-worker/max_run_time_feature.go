...
func (mrttf *MaxRunTimeTaskFeature) Start() *CommandExecutionError {
	...
	if time.Time(mrttf.task.Definition.Deadline).Before(time.Now().Round(0).Add(time.Second * time.Duration(mrttf.task.Payload.MaxRunTime))) {
		return MalformedPayloadError(fmt.Errorf("task deadline too close: deadline occurs in %v seconds, but maxRunTime is %vs", time.Time(mrttf.task.Definition.Deadline).Sub(time.Now().Round(0)).Seconds(), mrttf.task.Payload.MaxRunTime))
	}
