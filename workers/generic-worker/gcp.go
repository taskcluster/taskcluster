...
// startGCPTerminationPolling uses a hanging GET to detect GCP preemption.
// When preemption is detected, it triggers graceful termination.
func startGCPTerminationPolling() func() {
	...
	go func() {
		for {
			...
			log.Println("polling for GCP preemption (hanging GET)")
			client := &http.Client{}
			req, err := http.NewRequest("GET", GCPMetadataBaseURL+"/instance/preempted?wait_for_change=true", nil)
			if err != nil {
				log.Printf("WARNING: error creating GCP preemption request: %v", err)
				continue
			}
			req.Header.Add("Metadata-Flavor", "Google")
			resp, err := client.Do(req)
			if err != nil {
				log.Printf("WARNING: error checking GCP preemption: %v", err)
				continue
			}
			body, _ := io.ReadAll(resp.Body)
			resp.Body.Close()
			if string(body) == "TRUE" {
				log.Println("GCP Metadata Service says preemption is imminent")
