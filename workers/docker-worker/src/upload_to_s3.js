...
module.exports = async function uploadToS3 (
  ...
  compress)
{
  const tmpDir = await fsPromises.mkdtemp(`${os.tmpdir()}${sep}`);
  ...
  try {
    ...
    if (typeof source === 'string') {
      await fsPromises.writeFile(tmpFilePath, source);
      hash.update(source);
    } else {
    ...
    let stat = await fsPromises.stat(tmpFilePath);
    size = stat.size;
    ...
    fsPromises.rm(tmpDir, { recursive: true });
  }
