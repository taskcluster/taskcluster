...
Search results respect the caller's scopes: a caller holding `hooks:list-hooks:*`
sees hooks from all groups, while a caller holding only
`hooks:list-hooks:<hookGroupId>` for specific groups sees hooks from those groups
only. Callers without any `hooks:list-hooks:` scope receive an `InsufficientScopes`
error.
