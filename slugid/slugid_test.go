...
func TestSlugidV4(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		assert.Equal(match, true, fmt.Sprintf("Slug %q generated by v4() is invalid.", slug))
	}
}
...
func TestSlugidNice(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		assert.Equal(match, true, fmt.Sprintf("Slug %q generated by nice() is invalid.", slug))
	}
}
