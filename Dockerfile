...
RUN if [ -n "${DOCKER_FLOW_VERSION}" ]; then \
    ...
    echo \{\"version\": \"107.0.0\", \"commit\": \"local\", \"source\": \"https://github.com/taskcluster/taskcluster\", \"build\": \"NONE\"\} > version.json; \
fi
...
FROM node:24.19.0-alpine AS image
