...
go install github.com/taskcluster/generic-worker/win2012r2/update-worker-type
...
# query old instances
echo "$(date): Querying old instances of win2012r2..."
OLD_INSTANCES="$(aws ec2 describe-instances --filters Name=tag-key,Values=WorkerType Name=tag-value,Values=win2012r2 --query 'Reservations[*].Instances[*].InstanceId' --output text)"
...
aws ec2 create-tags --resources "${INSTANCE_ID}" --tags 'Key=WorkerType,Value=win2012r2'
echo "$(date): I've tagged it with \"WorkerType\": \"win2012r2\""
...
"${GOPATH}/bin/update-worker-type" "${IMAGE_ID}"
