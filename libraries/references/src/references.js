...
import { fromSerializable, makeSerializable } from './serializable.js';
import { readUriStructured, writeUriStructured } from './uri-structured.js';
...
export class References {
  ...
  /**
   * Create a new representation from the references generated by a service.
   * This is used within services' tests to validate that their references and
  ...
  static async fromService({ schemaset, references }) {
    ...
    const referenceFilename = (content) => {
      ...
      // later in validation)
      const schema = schemas.find(({ content: sch }) => sch.$id === content.$schema);
      const kind = schema ? schema.content.metadata.name : 'unknown';
  ...
  /**
   * Write out the references in the format that `yarn generate` expects for
   * the `generateReferences` target.
   */
  generateReferences() {
    process.stdout.write(JSON.stringify(this.asAbstract().makeSerializable(), null, 2));
  }
