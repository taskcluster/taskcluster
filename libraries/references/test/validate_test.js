        ...
        merge(
          {
            ...
            entries: entries.map(({ omitPaths = [], ...content }) =>
              omit(
                merge(
                  {
                    type: 'function',
...
suite(testing.suiteName(), function () {
  ...
  test('schema with no $id fails', async function () {
    new RefBuilder().then(async (references) => {
      references.schema({ omitPaths: ['$id'] }).end();
  ...
  test('schema with invalid $id fails', async function () {
    new RefBuilder().then(async (references) => {
      references.schema({ $id: '/schemas/foo.yml' }).end();
      assertProblems(references, [
  ...
  test('schema with invalid absolute $ref fails', async function () {
    new RefBuilder().then(async (references) => {
      references
        ...
        .end();
      assertProblems(references, ['schema test-schema.yml $ref at schema.properties.foo is not allowed']);
    });
  });
  ...
  test('schema with no metaschema fails', async function () {
    new RefBuilder().then(async (references) => {
      references.schema({ omitPaths: ['$schema'] }).end();
  ...
  test('invalid schema fails', async function () {
    new RefBuilder().then(async (references) => {
      references
        ...
        .end();
      assertProblems(references, ['test-schema.yml: schema/properties/abc must be object,boolean']);
    });
  });
  ...
  test('schema with "entries" but no "type" fails', async function () {
    new RefBuilder().then(async (references) => {
      references
        ...
        .end();
      assertProblems(references, ["test-schema.yml: schema has a 'entries' property but no 'type'"]);
    });
  });
  ...
  test('schema with "entries" but no "uniqueItems" fails', async function () {
    new RefBuilder().then(async (references) => {
      references
        ...
        .end();
      assertProblems(references, ["test-schema.yml: schema has a 'entries' property but no 'uniqueItems'"]);
    });
  });
  ...
  test('schema with "properties" but no "type" fails', async function () {
    new RefBuilder().then(async (references) => {
      references
        ...
        .end();
      assertProblems(references, ["test-schema.yml: schema has a 'properties' property but no 'type'"]);
    });
  });
  ...
  test('schema with "properties" but no "additionalProperties" fails', async function () {
    new RefBuilder().then(async (references) => {
      references
        ...
        .end();
      assertProblems(references, ["test-schema.yml: schema has a 'properties' property but no 'additionalProperties'"]);
    });
  });
  ...
  test('invalid schema with custom metaschema fails', async function () {
    new RefBuilder().then(async (references) => {
      references
        ...
        .end();
      assertProblems(references, ["test-schema.yml: schema/metadata must have required property 'name'"]);
    });
  });

  test('schema with undefined metaschema fails', async function () {
    new RefBuilder().then(async (references) => {
      references.schema({ $schema: '/schemas/nosuch.json#' }).end();
      assertProblems(references, [
  ...
  test('api reference with no $schema fails', async function () {
    new RefBuilder().then(async (references) => {
      references.apiref({ omitPaths: ['$schema'] }).end();
  ...
  test('exchanges reference with no $schema fails', async function () {
    new RefBuilder().then(async (references) => {
      references.exchangesref({ omitPaths: ['$schema'] }).end();
  ...
  test('invalid api reference fails', async function () {
    new RefBuilder().then(async (references) => {
      references.apiref({ serviceName: true }).end();
      assertProblems(references, ['test-api-ref.yml: reference/serviceName must be string']);
    });
  });

  test('invalid exchanges reference fails', async function () {
    new RefBuilder().then(async (references) => {
      references.schema({ $id: '/schemas/test/v2/message.json#' }).exchangesref({ title: false }).end();
      assertProblems(references, ['test-exch-ref.yml: reference/title must be string']);
    });
  });

  test('reference with undefined $schema fails', async function () {
    new RefBuilder().then(async (references) => {
      references.apiref({ $schema: '/schemas/nosuch.json#' }).end();
      assertProblems(references, ['reference test-api-ref.yml has invalid $schema (must be defined here)']);
    });
  });

  test('reference with non-metadata metaschema fails', async function () {
    new RefBuilder().then(async (references) => {
      references.apiref({ $schema: '/schemas/common/metadata-metaschema.json#' }).end();
      assertProblems(references, [
  ...
  test('exchanges reference with absolute entry schema URL fails', async function () {
    new RefBuilder().then(async (references) => {
      references.exchangesref({ entries: [{ schema: 'https://schemas.exmaple.com/message.json#' }] }).end();
      assertProblems(references, ['test-exch-ref.yml: entries[0].schema is not relative to the service']);
    });
  });
  ...
  test('exchanges reference with entry schema that does not exist fails', async function () {
    new RefBuilder().then(async (references) => {
      references.exchangesref({ entries: [{ schema: 'v2/message.json#' }] }).end();
      assertProblems(references, ['test-exch-ref.yml: entries[0].schema does not exist']);
    });
  });

  test('api reference with absolute entry input URL fails', async function () {
    new RefBuilder().then(async (references) => {
      references.apiref({ entries: [{ input: 'https://schemas.exmaple.com/resource.json#' }] }).end();
      assertProblems(references, ['test-api-ref.yml: entries[0].input is not relative to the service']);
    });
  });
  ...
  test('api reference with entry input that does not exist fails', async function () {
    new RefBuilder().then(async (references) => {
      references.apiref({ entries: [{ input: 'v2/resource.json#' }] }).end();
      assertProblems(references, ['test-api-ref.yml: entries[0].input does not exist']);
    });
  });

  test('api reference with absolute entry output URL fails', async function () {
    new RefBuilder().then(async (references) => {
      references.apiref({ entries: [{ output: 'https://schemas.exmaple.com/resource.json#' }] }).end();
      assertProblems(references, ['test-api-ref.yml: entries[0].output is not relative to the service']);
    });
  });
  ...
  test('api reference with entry output that does not exist fails', async function () {
    new RefBuilder().then(async (references) => {
      references.apiref({ entries: [{ output: 'v2/resource.json#' }] }).end();
      assertProblems(references, ['test-api-ref.yml: entries[0].output does not exist']);
    });
  });
  ...
  test('api reference with entry output that exists but has wrong $schema fails', async function () {
    new RefBuilder().then(async (references) => {
      references
        ...
        .end();
      assertProblems(references, ["test/v2/resource.json#'s $schema is not the metaschema"]);
    });
  });
