...
suite(testing.suiteName(), function () {
  suite('offset/limit', function () {
    ...
    test('paginate with limit higher than returned rows', async function () {
      assert.deepEqual(await paginateResults({ query: { limit: 10 }, fetch: fetcher(7) }), { rows: _.range(0, 7) });
    });

    test('paginate with limit equal to returned rows', async function () {
      assert.deepEqual(await paginateResults({ query: { limit: 7 }, fetch: fetcher(7) }), { rows: _.range(0, 7) }); // notably no continuationToken
    });

    test('paginate with limit less than returned rows', async function () {
      assert.deepEqual(await paginateResults({ query: { limit: 4 }, fetch: fetcher(7) }), {
        rows: _.range(0, 4),
        continuationToken: hashids.encode(4),
      });
    });

    test('paginate with continuationToken', async function () {
      const continuationToken = hashids.encode(5);
      assert.deepEqual(await paginateResults({ query: { limit: 100, continuationToken }, fetch: fetcher(7) }), {
        rows: _.range(5, 7),
      });
    });
    ...
    test('paginate with continuationToken and limit ending at returned rows', async function () {
      const continuationToken = hashids.encode(5);
      assert.deepEqual(await paginateResults({ query: { limit: 2, continuationToken }, fetch: fetcher(7) }), {
        rows: _.range(5, 7),
      });
    });
    ...
    test('paginate with continuationToken and limit returning another page', async function () {
      ...
      assert.deepEqual(await paginateResults({ query: { limit: 2, continuationToken }, fetch: fetcher(7) }), {
        rows: _.range(3, 5),
        continuationToken: hashids.encode(5),
      });
    });
  ...
  suite('index-based', function () {
    ...
    test('paginate with a limit higher than returned rows', async function () {
      assert.deepEqual(await paginateResults({ query: { limit: 110 }, indexColumns, fetch: fetcher(20, 5) }), {
        rows: data(20, 5).slice(0, 110),
      });
    });

    test('paginate with a limit equal to returned rows', async function () {
      assert.deepEqual(await paginateResults({ query: { limit: 100 }, indexColumns, fetch: fetcher(20, 5) }), {
        rows: data(20, 5),
      });
    });

    test('paginate with a limit one less than returned rows', async function () {
      assert.deepEqual(await paginateResults({ query: { limit: 99 }, indexColumns, fetch: fetcher(20, 5) }), {
        rows: data(20, 5).slice(0, 99),
        continuationToken: token(19, 3),
      });
      assert.deepEqual(
