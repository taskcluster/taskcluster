...
suite(testing.suiteName(), function () {
  ...
  suite('child monitors', function () {
    ...
    test('traceId becomes top-level', function () {
      const child = monitor.taskclusterPerRequestInstance({
        entryName: 'what',
        traceId: 'foo/bar',
        requestId: '123/456',
      });
  ...
  suite('uncaught and unhandled', function () {
    const testExits = (done, args, check) => {
      ...
      const proc = fork(path.resolve(__dirname, './exit.js'), args, {
        silent: true,
      });
    ...
    test('thrown but with no interception', function (done) {
      testExits(done, ['--shouldError'], (done, code, output) => {
        assert.equal(code, 1);
    ...
    test('thrown with interception', function (done) {
      testExits(done, ['--shouldError', '--patchGlobal'], (done, code, output) => {
        assert.equal(code, 1);
        assert(output.includes('Error: hello there'));
    ...
    test('unhandled but with no interception', function (done) {
      testExits(done, ['--shouldUnhandle'], (done, code, output) => {
        assert.equal(code, 1);
    ...
    test('unhandled with interception', function (done) {
      testExits(done, ['--shouldUnhandle', '--patchGlobal', '--bailOnUnhandledRejection'], (done, code, output) => {
        assert.equal(code, 1);
    ...
    test('unhandled with interception but continues', function (done) {
      testExits(done, ['--shouldUnhandle', '--patchGlobal'], (done, code, output) => {
        assert.equal(code, 0);
        assert(output.includes('whaaa'));
  ...
  suite('other basics', function () {
    ...
    test('should record first line of multiline errors', function () {
      ...
      assert(monitorManager.messages[0].Fields.stack.startsWith('Error: uhoh\nsomething went wrong\n..again\n    at '));
