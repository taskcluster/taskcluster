const {CryptoKeyring, SigningKeyring} = require('../src/Keyring');
const path = require('path');
const assert = require('assert').strict;

suite(path.basename(__filename), function() {
  suite('CryptoKeyring', function() {
  ...
  suite('SigningKeyring', function() {
    // NOTE: most of the shared Keyring logic is tested for CrytoKeyring and not
    // repeated here.
    //
    const azureSigningKey = 'super-secret';
    const pgSigningKey = 'really-secret';

    test('no keys', function() {
      const keyring = new SigningKeyring({});
      assert.throws(() => {
        keyring.currentKey('hmac-sha512');
      }, /no current signing key is configured/);
    });

    test('only azure', function() {
      const keyring = new SigningKeyring({azureSigningKey});
      const {id, algo, key} = keyring.currentKey('hmac-sha512');
      assert.equal(id, 'azure');
      assert.equal(algo, 'hmac-sha512');
      assert.equal(key.toString('utf8'), azureSigningKey);
    });

    test('non-string key', function() {
      assert.throws(
        () => new SigningKeyring({azureSigningKey: 42}),
        /must be a string/);
    });

    test('only pg', function() {
      const keyring = new SigningKeyring({dbSigningKeys: [{id: 'foo', algo: 'hmac-sha512', key: pgSigningKey}]});
      const {id, key} = keyring.currentKey('hmac-sha512');
      assert.equal(id, 'foo');
      assert.equal(key.toString('utf8'), pgSigningKey);
    });
  });
});
