...
| [0127](./0127.yml) | (pending release) | Remove the `else 0` fallback for priority `normal` in `queue_pending_tasks_add_for_task` |
| [0128](./0128.yml) | (pending release) | fix chained full outer join in queue_worker_stats that could report worker_count=0 for pools with pending but no claimed tasks |
