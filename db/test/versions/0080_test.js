...
suite(testing.suiteName(), function () {
  ...
  test('queue_workers', async function () {
    ...
    for (let i = 0; i < 10; i++) {
      await create(queueDb, { taskQueueId: `prov/w/${i}`, workerId: `worker-${i}` });
    }
    // create same worker with multiple queue ids
    const workerPoolIds = ['prov/w/1', 'some/other/pool'];
    await Promise.all(workerPoolIds.map(id => wmDb.fns.create_worker(
      id,
      'w/group',
      'worker-1',
      'static',
