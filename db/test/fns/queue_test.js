...
suite(testing.suiteName(), function () {
    ...
    for (const row of rows) {
      for (const run of row.runs) {
        for (const prop of ['scheduled', 'started', 'resolved', 'takenUntil']) {
          if (prop in run && typeof run[prop] === 'string' && !Number.isNaN(Date.parse(run[prop]))) {
            run[prop] = 'date';
          }
        }
      }
  ...
  suite('tests for pending tasks', function () {
    ...
    helper.dbTest('count empty queue', async function (db) {
      assert.deepEqual(await db.fns.queue_pending_tasks_count('tq1'), [{ queue_pending_tasks_count: 0 }]);
    });
    helper.dbTest('count queue containing messages', async function (db) {
      ...
      assert.deepEqual(await db.fns.queue_pending_tasks_count('tq1'), [{ queue_pending_tasks_count: 2 }]);
    });
    ...
    helper.dbTest('getting tasks on a queue by priority', async function (db) {
      await db.fns.queue_pending_tasks_add('tq1', 2, 'taskLowerPriority', 0, 'hint2', fromNow('20 seconds'));
      await db.fns.queue_pending_tasks_add('tq1', 0, 'taskDefaultPriority', 0, 'hint2', fromNow('20 seconds'));
      await db.fns.queue_pending_tasks_add('tq1', 9, 'taskHigherPriority', 0, 'hint1', fromNow('20 seconds'));
  ...
  suite('tests for claimed tasks', function () {
    ...
    helper.dbTest(
      'multiple rows for the same taskId,runId should exist but only visible returned',
      async function (db) {
  ...
  suite('task/task-group functions', function () {
    ...
    helper.dbTest('get_multiple_tasks with no such tasks', async function (db) {
      const res = await db.fns.get_multiple_tasks(JSON.stringify(['these', 'do', 'not', 'exist']), 1000, 0);
      assert.deepEqual(res, []);
    });
    ...
    helper.dbTest('get_multiple_tasks not an array', async function (db) {
      ...
      assert.rejects(
        db.fns.get_multiple_tasks(JSON.stringify({ 'tid-1': 'not an array' }), 1000, 0),
        (err) => err.code === INVALID_PARAMETER_VALUE,
      );
    });
    ...
    helper.dbTest('get_multiple_tasks works', async function (db) {
      ...
      // Limit to 2 results
      res = await db.fns.get_multiple_tasks(JSON.stringify(['tid-1', 'tid-2', 'doesnotexist', 'tid-5']), 2, 0);
      assert.equal(res.length, 2);
      expectedTasks = [(await db.fns.get_task_projid('tid-1'))[0], (await db.fns.get_task_projid('tid-2'))[0]];
      assert.deepEqual(res, expectedTasks);

      // Same limit, but with an offset
      res = await db.fns.get_multiple_tasks(JSON.stringify(['tid-1', 'tid-2', 'doesnotexist', 'tid-5']), 2, 2);
      assert.equal(res.length, 1);
      expectedTasks = [(await db.fns.get_task_projid('tid-5'))[0]];
      assert.deepEqual(res, expectedTasks);
    });
    ...
    suite('reclaim_task', function () {
      ...
      helper.dbTest('task with running run', async function (db) {
        ...
        assert.deepEqual(res, [
          {
            retries_left: 5,
            runs: [{ state: 'exception' }, { state: 'running', takenUntil: 'date' }],
            taken_until: takenUntil,
          },
        ]);
        const task = fixRuns(await db.fns.get_task_projid(taskId));
    ...
    suite('resolve_task', function () {
      ...
      helper.dbTest('task with running run', async function (db) {
        ...
        await create(db);
        await setTaskRuns(db, [{ state: 'exception' }, { state: 'running', takenUntil: oldTakenUntil.toJSON() }]);
    ...
    suite('check_task_claim', function () {
      ...
      helper.dbTest('task with running run, null task.takenUntil', async function (db) {
        ...
        await setTaskRuns(db, [{ state: 'running', takenUntil: takenUntil.toJSON() }]);
        const res = await db.fns.check_task_claim(taskId, 0, takenUntil);
        assert.deepEqual(res, []);
      });
      ...
      helper.dbTest('task with running run, mismatched run.takenUntil', async function (db) {
        await create(db);
        await setTaskTakenUntil(db, takenUntil);
        await setTaskRuns(db, [{ state: 'running', takenUntil: taskcluster.fromNow('1 hour').toJSON() }]);
        const res = await db.fns.check_task_claim(taskId, 0, takenUntil);
        assert.deepEqual(res, []);
      });
      ...
      helper.dbTest('task with running run', async function (db) {
        await create(db);
        await setTaskTakenUntil(db, takenUntil);
        await setTaskRuns(db, [{ state: 'exception' }, { state: 'running', takenUntil: takenUntil.toJSON() }]);
    ...
    suite('get_dependent_tasks', function () {
        ...
        await helper.withDbClient(async (client) => {
          for (const [dep, req, sat] of deps) {
            await client.query(
              ...
              values ($1, $2, 'all-completed', $3, now() + interval '1 day')`,
              [dep, req, sat],
            );
          }
        });
      ...
      helper.dbTest('filtering by satisfied', async function (db) {
        ...
        assert.deepEqual(
          res.map((row) => row.dependent_task_id),
          [satDep],
        );
  ...
  suite('queue_workers', function () {
    ...
    helper.dbTest('queue_worker_seen_with_last_date_active updates rows', async function (db) {
      ...
      const expireses = [taskcluster.fromNow('1 day'), taskcluster.fromNow('2 days')];
    ...
    helper.dbTest(
      'quarantine_queue_worker_with_last_date_active does nothing on nonexistent worker',
      async function (db) {
        const res = await db.deprecatedFns.quarantine_queue_worker_with_last_date_active(
          'prov/wt',
          'wg',
          'wi',
          new Date(),
    ...
    helper.dbTest(
      'quarantine_queue_worker_with_last_date_active updates quarantine date + expires, returns row',
      async function (db) {
        await create(db);
    ...
    helper.dbTest(
      'quarantine_queue_worker_with_last_date_active_and_details updates quarantine date + expires, returns row',
      async function (db) {
        await create(db);
        const quarantineUntil = taskcluster.fromNow('1 year');
        const res = await db.fns.quarantine_queue_worker_with_last_date_active_and_details(
          'prov/wt',
          'wg',
          'wi',
          quarantineUntil,
  ...
  suite('task_queues', function () {
    ...
    helper.dbTest('create_task_queue (deprecated)', async function (db) {
      await db.deprecatedFns.create_task_queue('prov/wt', expires, new Date(), 'desc', 'unstable');
  ...
  suite('queue_worker_stats method', function () {
    ...
    helper.dbTest('returns correct stats when some tables have data', async function (db) {
      ...
      await db.fns.queue_claimed_task_put('claimed-task-id', 0, fromNow('1 hour'), taskQueueId, 'wg1', 'worker1');
  ...
  suite('priority change helpers', function () {
    ...
    helper.dbTest('queue_change_task_priority skips resolved or expired tasks', async function (db) {
      ...
      await helper.withDbClient(async (client) => {
        await client.query(
          "update tasks set ever_resolved = false, deadline = now() - interval '5 minutes' where task_id = $1",
          [id],
        );
      });
