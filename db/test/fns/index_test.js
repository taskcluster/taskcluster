...
suite(testing.suiteName(), function() {
  ...
  setup('reset table', async function() {
    await helper.withDbClient(async client => {
      ...
      await client.query('delete from namespaces');
    });
    helper.fakeDb.fakeindex.reset();
  });

  const create_namespace = async (db, wp = {}) => {
    await db.fns.create_namespace(
      wp.parent || 'par/ent',
      wp.name || 'name',
      wp.expires || fromNow('1 day'),
    );
  };
  const update_namespace = async (db, t = {}, etag) => {
    return await db.fns.update_namespace(
      t.parent || 'par/ent',
      t.name || 'name',
      t.expires || new Date(),
      etag,
  ...
  suite(`${testing.suiteName()} - namespaces`, function() {
    helper.dbTest('create_namespace/get_namespace', async function(db, isFake) {
      const now = new Date();
      const taskId = slug.nice();
      await create_namespace(db, { taskId });

      const rows = await db.fns.get_namespace('par/ent', 'name');
      assert.equal(rows[0].parent, 'par/ent');
      assert.equal(rows[0].name, 'name');
      assert(rows[0].expires > now);
    });

    helper.dbTest('get_namespace does not omit expired indexed tasks', async function(db, isFake) {
      const now = new Date();
      const taskId = slug.nice();
      await create_namespace(db, { expires: now, taskId });

      const rows = await db.fns.get_namespace('par/ent', 'name');
      assert.equal(rows.length, 1);
    });

    helper.dbTest('get_namespace not found', async function(db, isFake) {
      const rows = await db.fns.get_namespace('par/ent', 'name');
      assert.deepEqual(rows, []);
    });

    helper.dbTest('get_namespaces empty', async function(db, isFake) {
      const rows = await db.fns.get_namespaces(null, null, null, null);
      assert.deepEqual(rows, []);
    });

    helper.dbTest('get_namespaces full, pagination', async function(db, isFake) {
      const oneDay = fromNow('1 day');
      for (let i = 0; i < 10; i++) {
        await create_namespace(db, {
          parent: `parent/${i}`,
          name: `name/${i}`,
          expires: oneDay,
        });
      ...
      let rows = await db.fns.get_namespaces(null, null, null, null);
      assert.deepEqual(
        rows.map(r => ({ parent: r.parent, name: r.name })),
        _.range(10).map(i => ({ parent: `parent/${i}`, name: `name/${i}` })));
      assert.deepEqual(rows[0].expires, oneDay);

      rows = await db.fns.get_namespaces(null, null, 2, 4);
      assert.deepEqual(
        rows.map(r => ({ parent: r.parent, name: r.name })),
        [4, 5].map(i => ({ parent: `parent/${i}`, name: `name/${i}` })));
    });

    helper.dbTest('get_namespaces only returns non expired tasks', async function(db, isFake) {
      const oneDay = fromNow('1 day');
      await create_namespace(db, {
        parent: `parent/1`,
        name: `name/1`,
    ...
    helper.dbTest('expire_namespaces', async function(db, isFake) {
      await create_namespace(db, { parent: 'parent/1', expires: fromNow('- 1 day') });
      await create_namespace(db, { parent: 'parent/2', expires: fromNow('- 2 days') });
      await create_namespace(db, { parent: 'parent/3', expires: fromNow('1 day') });

      const count = (await db.fns.expire_namespaces())[0].expire_namespaces;
      assert.equal(count, 2);
      const rows = await db.fns.get_namespaces(null, null, null, null);
      assert.equal(rows.length, 1);
    });

    helper.dbTest('update_namespace, change to a single field', async function(db, isFake) {
      const etag = await create_namespace(db);
      await db.fns.update_namespace(
        'par/ent',
        'name',
        null,
        etag,
    ...
    helper.dbTest('update_namespace, change to a multiple fields', async function(db, isFake) {
      const etag = await create_namespace(db);
      const updated = await db.fns.update_namespace(
        'par/ent',
        'name',
        null,
        etag,
    ...
    helper.dbTest('update_namespace, no changes', async function(db, isFake) {
      const etag = await create_namespace(db);
      const updated = await db.fns.update_namespace(
        'par/ent',
        'name',
    ...
    helper.dbTest('update_namespace, indexed task doesn\'t exist', async function(db, isFake) {
      const etag = await create_namespace(db);

      await assert.rejects(
        async () => {
          await update_namespace(db, { parent: 'does-not-exist' }, etag);
        },
        /no such row/,
      );
    });

    helper.dbTest('update_namespace, override when etag not specified', async function(db, isFake) {
    ...
    helper.dbTest('update_namespace, throws when etag is wrong', async function(db, isFake) {
      await create_namespace(db);
      await assert.rejects(
        async () => {
          await db.fns.update_namespace(
