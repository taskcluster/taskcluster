...
| create_worker_pool_error | write | error_id_in text, worker_pool_id_in text, reported_in timestamptz, kind_in text, title_in text, description_in text, extra_in jsonb | uuid | Create a new worker pool error.  Raises UNIQUE_VIOLATION if the pool already exists. |
...
| expire_worker_pool_errors | write | expires_in timestamptz | integer | Expire worker pool errors reported before `expires_in`. Returns a count of rows that have been deleted. |
...
| get_worker_pool_error | read | error_id_in text, worker_pool_id_in text | table(error_id text, worker_pool_id text, reported timestamptz, kind text, title text, description text, extra jsonb) | Get an existing worker pool error. The returned table will have one or (if the specified worker pool error exists) zero rows. |
| get_worker_pool_errors | read | page_size_in integer, page_offset_in integer | table(error_id text, worker_pool_id text, reported timestamptz, kind text, title text, description text, extra jsonb) | Get existing worker pool errors, ordered by `error_id`.  If the pagination arguments are both NULL, all rows are returned.<br />Otherwise, page_size rows are returned at offset page_offset. |
