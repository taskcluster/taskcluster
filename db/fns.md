...
| get_worker_pool_errors | read | page_size_in integer, page_offset_in integer | table(error_id text, worker_pool_id text, reported timestamptz, kind text, title text, description text, extra jsonb) | Get existing worker pool errors, ordered by `reported` date.  If the pagination arguments are both NULL, all rows are returned.<br />Otherwise, page_size rows are returned at offset page_offset. |
