...
func FormatSourceAndSave(sourceFile string, sourceCode []byte) {
	...
	formattedContent = regexp.MustCompile(`github\.com/taskcluster/taskcluster/v[0-9]+/`).ReplaceAll(formattedContent, []byte("github.com/taskcluster/taskcluster/v70/"))
...
func (apiDefs APIDefinitions) GenerateCode(goOutputDir string) {
	...
	for i := range apiDefs {
		if strings.Contains(apiDefs[i].PackageName, "events") {
			amqpApiLinks += "\n" + "* https://pkg.go.dev/github.com/taskcluster/taskcluster/v70/clients/client-go/" + apiDefs[i].PackageName + "\n"
