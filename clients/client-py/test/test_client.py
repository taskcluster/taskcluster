...
def apiRef(mocker):
    ...
    topicEntry = base.createApiEntryTopicExchange(
        "topicName", "topicExchange", routingKey=keys
    )
    entries = [
...
def test_no_args(client):
    assert ({}, None, {}, None, None) == client._processArgs(
        {"args": [], "name": "test"}
    )
...
def test_finds_payload(client):
    ...
    actual = client._processArgs(
        {"args": [], "name": "test", "input": True}, {"a": 123}
    )
...
def test_invalid_too_many_positional_args(client):
    with pytest.raises(exc.TaskclusterFailure):
        client._processArgs(
            {"args": ["test"], "name": "test"}, "enough", "one too many"
        )
...
def test_calling_convention_1_with_payload(client):
    params, payload, query, _, _ = client._processArgs(
        {"args": ["k1", "k2"], "name": "test", "input": True}, 1, 2, {"A": 123}
    )
    assert params == {"k1": 1, "k2": 2}
...
def test_calling_convention_2_without_payload(client):
    params, payload, query, _, _ = client._processArgs(
        {"args": ["k1", "k2"], "name": "test"}, k1=1, k2=2
    )
    assert params == {"k1": 1, "k2": 2}
    assert payload is None
    assert query == {}
...
def test_calling_convention_2_with_payload(client):
    params, payload, query, _, _ = client._processArgs(
        {"args": ["k1", "k2"], "name": "test", "input": True}, {"A": 123}, k1=1, k2=2
    )
    assert params == {"k1": 1, "k2": 2}
    assert payload == {"A": 123}
    assert query == {}
...
def test_calling_convention_3_without_payload_without_query(client):
    params, payload, query, _, _ = client._processArgs(
        {"args": ["k1", "k2"], "name": "test"}, params={"k1": 1, "k2": 2}
    )
    assert params == {"k1": 1, "k2": 2}
    assert payload is None
    assert query == {}
...
def test_calling_convention_3_with_pos_args_same_as_param_kwarg_dict_vals_with_payload_with_query(
    client,
):
    with pytest.raises(exc.TaskclusterFailure):
        params, payload, query, _, _ = client._processArgs(
...
def test_success_first_try_payload(client, apiPath):
    with mock.patch.object(utils, "makeSingleHttpRequest") as p:
        ...
        p.assert_called_once_with(
            "GET", apiPath, utils.dumpJson({"payload": 2}), mock.ANY
        )
        assert expected == v
...
def test_build_url_query_string(client, apiPath_2):
    actual = client.buildUrl(
        "two_args_no_input", params={"arg0": "arg0", "arg1": "arg1"}, query={"qs0": 1}
    )
...
def test_no_creds_needed():
    ...
    with httmock.HTTMock(auth_response):
        client = subject.Auth(
            {"rootUrl": "https://tc-tests.example.com", "credentials": {}}
        )
...
def test_permacred_simple_authorizedScopes():
    ...
    assert result == {
        "scopes": ["assume:anonymous", "test:a", "test:b"],
        "clientId": "tester",
    }
...
def test_temporary_credentials_authorizedScopes():
    ...
    assert result == {
        "scopes": ["assume:anonymous", "test:xyz:abc"],
        "clientId": "tester",
    }
...
def test_signed_url():
    ...
    assert response == {
        "scopes": sorted(["assume:anonymous", "test:*", "auth:create-client:test:*"]),
        "clientId": "tester",
    }
