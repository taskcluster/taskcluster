...
import taskcluster
from taskcluster import download, upload
...
def test_getUrl_download_fails():
    ...
    with httptest.Server(Server) as ts:
        ...
        with pytest.raises(aiohttp.ClientResponseError):
            download.downloadToBuf(name="some/object", objectService=objectService)
        assert getcount == 1
...
def test_download(randbytes):
    ...
    with httptest.Server(Server) as ts:
        ...
        download.download(
            name="some/object", writerFactory=writerFactory, objectService=objectService
        )
