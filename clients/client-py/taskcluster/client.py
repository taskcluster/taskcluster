...
class BaseClient:
    ...
    def _makeTopicExchange(self, entry, *args, **kwargs):
        ...
        data = {
            "exchange": "{}/{}".format(
                self.options["exchangePrefix"].rstrip("/"),
        ...
        for key in entry["routingKey"]:
            if "constant" in key:
                ...
                log.debug(
                    "Did not find %s in input params, using %s", key["name"], value
                )
    ...
    def buildUrl(self, methodName, *args, **kwargs):
        ...
        return liburls.api(
            self.options["rootUrl"], self.serviceName, self.apiVersion, route
        )
    ...
    def _processArgs(self, entry, *_args, **_kwargs):
        ...
        for arg in args:
            if not isinstance(arg, str) and not isinstance(arg, int):
                raise exceptions.TaskclusterFailure(
                    'Positional arg "{}" to {} is not a string or int'.format(
                        arg, entry["name"]
                    )
                )
        ...
        for name, arg in kwApiArgs.items():
            if not isinstance(arg, str) and not isinstance(arg, int):
                raise exceptions.TaskclusterFailure(
                    'KW arg "{}: {}" to {} is not a string or int'.format(
                        name, arg, entry["name"]
                    )
                )
        ...
        if len(reqArgs) > len(args) + len(kwApiArgs):
            raise exceptions.TaskclusterFailure(
                f"{entry['name']} takes {len(reqArgs)} args, only {len(args) + len(kwApiArgs)} were given"
            )
        ...
        if len(reqArgs) != len(routeParams):
            errMsg = "{} takes {} args, {} given".format(
        ...
        for reqArg in reqArgs:
            if reqArg not in routeParams:
                errMsg = '{} requires a "{}" argument which was not provided'.format(
                    entry["name"], reqArg
                )
                log.error(errMsg)
                raise exceptions.TaskclusterFailure(errMsg)
    ...
    def _subArgsInRoute(self, entry, args):
        ...
        for arg, val in args.items():
            ...
            if toReplace not in route:
                raise exceptions.TaskclusterFailure(
                    "Arg {} not found in route for {}".format(arg, entry["name"])
                )
    ...
    def _hasCredentials(self):
        ...
        return (
            cred
            and "clientId" in cred
            and "accessToken" in cred
            and cred["clientId"]
            and cred["accessToken"]
        )
