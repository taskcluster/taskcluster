...
builder.declare(
  {
    ...
    title: 'List Tasks',
    category: 'Index Service',
    description: ['(a version of listTasks with POST for backward compatibility; do not use)'].join('\n'),
  },
  async function (req, res) {
    const namespace = req.params.namespace || '';
    await req.authorize({
...
builder.declare(
  ...
  async function (req, res) {
    ...
    // Insert task
    return helpers.taskUtils.insertTask(this.db, namespace, input).then(function (task) {
...
builder.declare(
  ...
  async function (req, res) {
    ...
    if (isPublic) {
      ...
      const url = that.queue.externalBuildUrl(that.queue.getLatestArtifact, task.taskId, artifactName);
      return res.redirect(303, url);
    }

    const url = that.queue.externalBuildSignedUrl(that.queue.getLatestArtifact, task.taskId, artifactName, {
      expiration: 15 * 60,
    });
    return res.redirect(303, url);
  },
