...
const load = loader(
  {
        ...
        libReferences
          .fromService({
            ...
            references: [
              builder.reference(),
              exchanges.reference(),
              MonitorManager.reference('notify'),
              MonitorManager.metricsReference('notify'),
            ],
          })
    ...
    denier: {
      requires: ['cfg', 'db'],
      setup: ({ cfg, db }) => new Denier({ emailBlacklist: cfg.app.emailBlacklist, db: db }),
    },
    ...
    notifier: {
      ...
      setup: ({ cfg, publisher, rateLimit, ses, denier, monitor, matrix, slack }) =>
        new Notifier({
          denier,
          publisher,
          rateLimit,
