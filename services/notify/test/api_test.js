...
helper.secrets.mockSuite(testing.suiteName(), ['aws'], function(mock, skipping) {
  ...
  test('pulse', async function() {
    await serverHelper.apiClient.pulse({ routingKey: 'notify-test', message: { test: 123 } });
    pulseHelper.assertPulseMessage('notification', m => (
  ...
  test('pulse() fails if pulse publish fails', async function() {
    ...
    const apiClient = serverHelper.apiClient.use({ retries: 0 });
  ...
  test('does not send notifications to denylisted pulse address', async function() {
    ...
    await serverHelper.apiClient.addDenylistAddress({
  ...
  test('email', async function() {
    await serverHelper.apiClient.email({
  ...
  test('matrix', async function() {
    await serverHelper.apiClient.matrix({ body: 'Does this work?', roomId: '!foobar:baz.com', msgtype: 'm.text' });
    assert.equal(matrixHelper.matrixClient.sendEvent.callCount, 1);
    assert.equal(matrixHelper.matrixClient.sendEvent.args[0][0], '!foobar:baz.com');
    assert.equal(matrixHelper.matrixClient.sendEvent.args[0][2].body, 'Does this work?');
  ...
  test('matrix (default msgtype)', async function() {
    await serverHelper.apiClient.matrix({ body: 'Does this work?', roomId: '!foobar:baz.com' });
    assert.equal(matrixHelper.matrixClient.sendEvent.callCount, 1);
    assert.equal(matrixHelper.matrixClient.sendEvent.args[0][0], '!foobar:baz.com');
    assert.equal(matrixHelper.matrixClient.sendEvent.args[0][2].body, 'Does this work?');
  ...
  test('matrix (rejected)', async function() {
    try {
      await serverHelper.apiClient.matrix({ body: 'Does this work?', roomId: '!rejected:baz.com' });
      ...
      assert.equal(matrixHelper.matrixClient.sendEvent.callCount, 1);
      assert.equal(matrixHelper.matrixClient.sendEvent.args[0][0], '!rejected:baz.com');
    }
  });

  test('matrix (denylisted)', async function() {
    await serverHelper.apiClient.addDenylistAddress({ notificationType: 'matrix-room', notificationAddress: '!foo:baz.com' });
    try {
      await serverHelper.apiClient.matrix({ body: 'Does this work?', roomId: '!foo:baz.com' });
      throw new Error('should have failed');
    } catch (err) {
  ...
  test('slack', async function() {
    await serverHelper.apiClient.slack({ channelId: 'C123456', text: 'Does this work?' });
  ...
  test('slack (denylisted)', async function() {
    await serverHelper.apiClient.addDenylistAddress({ notificationType: 'slack-channel', notificationAddress: 'C123456' });
    try {
      await serverHelper.apiClient.slack({ channelId: 'C123456', text: 'Does this work?' });
      throw new Error('should have failed');
    } catch (err) {
  ...
  test('Denylist: addDenylistAddress()', async function() {
    ...
    await serverHelper.apiClient.addDenylistAddress(dummyAddress1);

    // Check that the address was successfully added
    await dbHelper.db.fns.add_denylist_address(
      dummyAddress1.notificationType,
      dummyAddress1.notificationAddress,
    );
    let existsTable = await dbHelper.db.fns.exists_denylist_address(
      dummyAddress1.notificationType,
      dummyAddress1.notificationAddress,
    );
  ...
  test('Denylist: deleteDenylistAddress()', async function() {
    // Add some items
    await serverHelper.apiClient.addDenylistAddress(dummyAddress1);
    await serverHelper.apiClient.addDenylistAddress(dummyAddress2);

    // Make sure they are added
    let items = await dbHelper.db.fns.all_denylist_addresses(1000, 0);
    assert(items.length, 2);

    // Remove an item and check for success
    await serverHelper.apiClient.deleteDenylistAddress(dummyAddress1);
    items = await dbHelper.db.fns.all_denylist_addresses(1000, 0);
  ...
  test('Denylist: listDenylist()', async function() {
    ...
    let addressList = await serverHelper.apiClient.listDenylist();
    ...
    addressList = await serverHelper.apiClient.listDenylist();
