...
export default ({ workerManager }, _isAuthed, _rootUrl, _monitor, _strategies, _req, _cfg, _requestId) => {
  const WorkerManagerWorkerPoolSummaries = new ConnectionLoader(async ({ filter, options }) => {
  ...
  const WorkerManagerErrors = new ConnectionLoader(async ({ workerPoolId, launchConfigId, filter, options }) => {
    if (launchConfigId) {
      options.launchConfigId = launchConfigId;
    }
  ...
  const WorkerManagerProviders = new ConnectionLoader(async ({ filter, options }) => {
