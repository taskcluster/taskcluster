...
builder.declare(
  ...
  async function (req, res) {
    ...
    await this.db.fns.upsert_secret(
      name,
      this.db.encrypt({
...
builder.declare(
  {
    ...
    stability: 'stable',
    category: 'Secrets Service',
    description: ['Delete the secret associated with some key. It will succeed whether or not the secret exists'].join(
      '\n',
    ),
  },
  async function (req, res) {
    ...
    await this.db.fns.insert_secrets_audit_history(name, await req.clientId(), AUDIT_ENTRY_TYPE.SECRET.DELETED);
...
builder.declare(
  ...
  async function (req, res) {
    const { continuationToken, rows: secrets } = await paginateResults({
      query: req.query,
      fetch: (size, offset) => this.db.fns.get_secrets(size, offset),
    });
