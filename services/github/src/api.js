  ...
  async function (req, res) {
    ...
    if (xHubSignature && !webhookSecrets) {
      ...
      if (!webhookSecrets.some(webhookSecret => checkGithubSignature(webhookSecret, bodyPayload, xHubSignature))) {
        ...
        await this.monitor.reportError(new Error('X-hub-signature does not match'), {
          xHubSignature,
          installationId,
    ...
    try {
      ...
      switch (eventType) {
        ...
        case EVENT_TYPES.INSTALLATION:
          ...
          await this.db.fns.upsert_github_integration(body.installation.account.login, installationId);
...
builder.declare(
  ...
  async function (req, res) {
    ...
    if (repository && !organization) {
      return res.reportError('InputError', 'Must provide `organization` if querying `repository`', {});
    }
    if (sha && !repository) {
      return res.reportError('InputError', 'Must provide `repository` if querying `sha`', {});
    }
    if (pullRequest && !repository) {
      return res.reportError('InputError', 'Must provide `repository` if querying `pullRequest`', {});
    }

    const { continuationToken, rows: builds } = await paginateResults({
      query: req.query,
      fetch: (size, offset) =>
        this.db.fns.get_github_builds_pr(
          size,
          offset,
          organization || null,
          repository || null,
...
builder.declare(
  {
    ...
    description: ['Cancel all running Task Groups associated with given repository and sha or pullRequest number'].join(
      '\n'
    ),
  },
  async function (req, res) {
    ...
    const builds = await this.db.fns.get_pending_github_builds(null, null, organization, repository, sha, pullRequest);
    ...
    try {
      ...
      await Promise.all(
        taskGroupIds.map(taskGroupId => this.db.fns.set_github_build_state(taskGroupId, GITHUB_BUILD_STATES.CANCELLED))
      );
    } catch (err) {
...
builder.declare(
  {
    ...
    description: ['Returns any repository metadata that is', 'useful within Taskcluster related services.'].join('\n'),
    stability: 'experimental',
    method: 'get',
    category: 'Github Service',
...
builder.declare(
  {
    ...
    description: ['For a given Issue or Pull Request of a repository, this will write a new message.'].join('\n'),
    stability: 'stable',
    method: 'post',
    category: 'Github Service',
    // route and input (schema) matches github API
...
builder.declare(
  ...
  async function (req, res) {
    const {
      ...
      repository = 'testing',
      fakeEvent: { type: fakeEventType, action: fakeEventAction, overrides: fakeEventData } = {},
    } = req.body;
