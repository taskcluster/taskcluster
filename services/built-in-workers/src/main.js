...
const load = loader(
  {
    ...
    generateReferences: {
      ...
      setup: async ({ cfg }) =>
        libReferences
          .fromService({
            references: [
              MonitorManager.reference('built-in-workers'),
              MonitorManager.metricsReference('built-in-workers'),
            ],
          })
    ...
    server: {
      ...
      setup: async ({ failTaskQueue, succeedTaskQueue }) => {
        await Promise.all([succeedTaskQueue.runWorker(), failTaskQueue.runWorker()]);
      },
    },
  },
  {
