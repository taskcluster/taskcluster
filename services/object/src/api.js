 ...
 * methods)
 */
export const DOWNLOAD_METHODS = ['getUrl', 'simple'];
...
builder.declare(
  ...
  async function (req, res) {
    ...
    if (!backend) {
      return res.reportError('NoMatchingBackend', 'No backend matched the given name and projectId', {});
    }
    ...
    try {
      ...
      if (err.code === UNIQUE_VIOLATION) {
        return res.reportError('RequestConflict', 'Object already exists', {
          name,
          projectId,
    ...
    try {
      ...
      if (err.code === UNIQUE_VIOLATION) {
        return res.reportError('RequestConflict', 'Content hash already exists with a different value', { name });
      }
      throw err;
    }
...
builder.declare(
  ...
  async function (req, res) {
    ...
    if (object.project_id !== projectId) {
      return res.reportError('InputError', 'Object "{{name}}" does not have projectId {{projectId}}', {
        name,
        projectId,
      });
    }
    ...
    if (object.upload_id !== uploadId) {
      return res.reportError('RequestConflict', 'Object "{{name}}" does not have uploadId {{uploadId}}', {
        name,
        uploadId,
      });
    }
    ...
    if (!backend) {
      return res.reportError('NoMatchingBackend', 'The backend for this object is no longer defined', {});
    }
...
builder.declare(
  ...
  async function (req, res) {
    ...
    const matchingMethods = DOWNLOAD_METHODS.filter((m) => backendMethods.includes(m) && callerMethods.includes(m));

    if (matchingMethods.length < 1) {
      return res.reportError('NoMatchingMethod', 'Object supports methods {{methods}}', {
