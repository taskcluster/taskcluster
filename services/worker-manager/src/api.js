...
const builder = new APIBuilder({
  ...
  context: ['cfg', 'db', 'providers', 'publisher', 'monitor', 'notify'],
});
...
builder.declare(
  {
    ...
    description: ['Retrieve a list of providers that are available for worker pools.'].join('\n'),
  },
  function (req, res) {
...
builder.declare(
  {
    ...
    description: ['Create a new worker pool. If the worker pool already exists, this will throw an error.'].join('\n'),
  },
  async function (req, res) {
    ...
    try {
      ...
      const message = err.message.includes('Launch config') ? err.message : 'Worker pool already exists';
...
builder.declare(
  ...
  async function (req, res) {
    ...
    const newConfig = workerPool.config?.launchConfigs
      ? { ...workerPool.config, launchConfigs: [] }
      : workerPool.config;
...
builder.declare(
  ...
  async function (req, res) {
    ...
    const { continuationToken, rows } = await paginateResults({
      query: req.query,
      fetch: (size, offset) =>
        this.db.fns.get_worker_pool_launch_configs(workerPoolId, includeArchived ? null : false, size, offset),
    });
...
builder.declare(
  {
    ...
    stability: APIBuilder.stability.stable,
    output: 'worker-pool-full.yml',
    description: ['Fetch an existing worker pool defition.'].join('\n'),
  },
  async function (req, res) {
    const { workerPoolId } = req.params;
...
builder.declare(
  {
    ...
    description: ['Get the list of all the existing worker pools.'].join('\n'),
  },
  async function (req, res) {
    const { continuationToken, rows } = await paginateResults({
      query: req.query,
...
builder.declare(
  {
    ...
    description: ['Get the list of worker pool errors.'].join('\n'),
  },
  async function (req, res) {
    const { workerPoolId } = req.params;
    ...
    const { continuationToken, rows } = await paginateResults({
      query: req.query,
      fetch: (size, offset) =>
        this.db.fns.get_worker_pool_errors_for_worker_pool2(
...
declareWithTrailingColon(
  {
    ...
    category: 'Workers',
    description: ['Get the list of all the existing workers in a given group in a given worker pool.'].join('\n'),
  },
  async function (req, res) {
    ...
    const { rows, continuationToken } = await paginateResults({
      query: req.query,
      fetch: (size, offset) =>
        this.db.fns.get_worker_manager_workers2(workerPoolId, workerGroup, null, null, null, size, offset),
    });
...
declareWithTrailingColon(
  {
    ...
    category: 'Workers',
    description: ['Get a single worker.'].join('\n'),
  },
  async function (req, res) {
...
declareWithTrailingColon(
  ...
  async function (req, res) {
    ...
    const workerPool = await WorkerPool.get(this.db, workerPoolId);
    if (!workerPool) {
      return res.reportError('ResourceNotFound', `Worker pool ${workerPoolId} does not exist`, {});
    }
...
builder.declare(
  ...
  async function (req, res) {
    ...
    if (!provider) {
      return res.reportError('ResourceNotFound', `Provider ${worker.providerId} for this worker does not exist`, {});
    } else if (provider.setupFailed) {
      return res.reportError('ProviderSetupFailed', 'Provider backend may be down or misconfigured', {
...
builder.declare(
  {
    ...
    stability: APIBuilder.stability.stable,
    output: 'worker-list.yml',
    description: ['Get the list of all the existing workers in a given worker pool.'].join('\n'),
  },
  async function (req, res) {
    const { workerPoolId } = req.params;
    const workerPool = await WorkerPool.get(this.db, workerPoolId);
    ...
    const { rows, continuationToken } = await paginateResults({
      query: req.query,
      fetch: (size, offset) =>
        this.db.fns.get_worker_manager_workers2({
...
builder.declare(
  ...
  async function (req, res) {
    ...
    if (!provider) {
      ...
      return res.reportError('ResourceNotFound', `Provider ${providerId} does not exist`, {});
    } else if (provider.setupFailed) {
      return res.reportError('ProviderSetupFailed', 'Provider backend may be down or misconfigured', {
        providerId,
      });
...
builder.declare(
  ...
  async function (req, res) {
    ...
    const { terminateAfter } = Provider.interpretLifecycle({
      lifecycle: {
...
builder.declare(
  ...
  async function (req, res) {
    ...
    const { rows: workers, continuationToken } = await Worker.getWorkers(this.db, { workerPoolId }, req.query);
