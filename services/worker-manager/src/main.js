...
import { Worker, WorkerPool, WorkerPoolError, WorkerPoolLaunchConfig } from './data.js';
...
const load = loader(
  {
        ...
        libReferences
          .fromService({
            ...
            references: [
              builder.reference(),
              exchanges.reference(),
              MonitorManager.reference('worker-manager'),
              MonitorManager.metricsReference('worker-manager'),
            ],
          })
    ...
    api: {
      requires: ['cfg', 'db', 'schemaset', 'monitor', 'providers', 'publisher', 'notify'],
      setup: async ({ cfg, db, schemaset, monitor, providers, publisher, notify }) => {
