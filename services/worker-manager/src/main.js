...
let load = loader({
  ...
  providers: {
    requires: ['cfg', 'monitor', 'notify', 'db', 'estimator', 'schemaset', 'publisher'],
    setup: async ({ cfg, monitor, notify, db, estimator, schemaset, publisher }) =>
      new Providers().setup({
        cfg, monitor, notify, db, estimator, publisher,
