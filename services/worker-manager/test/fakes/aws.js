...
export class FakeEC2 extends FakeCloud {
  ...
  runInstances(r) {
    this.mock.on(RunInstancesCommand).callsFake((launchConfig) => {
  ...
  describeRegions() {
    this.mock.on(DescribeRegionsCommand).resolves({
      Regions: [{ RegionName: 'us-west-2' }, { RegionName: 'us-east-1' }, { RegionName: 'eu-central-1' }],
    });
  }

  terminateInstances() {
    this.mock.on(TerminateInstancesCommand).callsFake(({ InstanceIds }) => {
      return {
  ...
  describeInstanceStatus() {
    this.mock.on(DescribeInstanceStatusCommand).callsFake(({ InstanceIds }) => {
