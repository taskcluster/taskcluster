...
helper.secrets.mockSuite(testing.suiteName(), [], function(mock, skipping) {
  ...
  test('ping', () => {
    return serverHelper.apiClient.ping();
  });

  test('allPurgeRequests without scopes', async function() {
    const client = new serverHelper.PurgeCacheClient({ rootUrl: helper.rootUrl });
  ...
  test('Publish purge-cache requests', async () => {
    ...
    let openRequests = await serverHelper.apiClient.allPurgeRequests();
    ...
    await serverHelper.apiClient.purgeCache('dummy-provisioner-extended-extended/dummy-worker-extended-extended', {
      cacheName: 'my-test-cache',
    });

    // Check that the first request is valid
    openRequests = await serverHelper.apiClient.allPurgeRequests();
    ...
    await serverHelper.apiClient.purgeCache('dummy-provisioner-extended-extended-2/dummy-worker-extended-extended', {
      cacheName: 'my-test-cache',
    });
    await serverHelper.apiClient.purgeCache('dummy-provisioner-extended-extended/dummy-worker-extended-extended-2', {
      cacheName: 'my-test-cache',
    });
    openRequests = await serverHelper.apiClient.allPurgeRequests();
    assume(openRequests.requests.length).equals(4);

    let spec = await serverHelper.apiClient.purgeRequests('dummy-provisioner-extended-extended-2/dummy-worker-extended-extended');
    assume(spec.requests.length).equals(1);
    spec = await serverHelper.apiClient.purgeRequests('dummy-provisioner-extended-extended/dummy-worker-extended-extended-2');
    assume(spec.requests.length).equals(1);
    spec = await serverHelper.apiClient.purgeRequests('dummy-provisioner-extended-extended/dummy-worker-extended-extended');
    assume(spec.requests.length).equals(2);

    // Finally we try with since included
    spec = await serverHelper.apiClient.purgeRequests(
  ...
  test('purgeRequest caching', async function() {
    sinon.stub(dbHelper.db.fns, 'purge_requests_wpid').callsFake(async (query) => []);
    try {
      const since = taskcluster.fromNow('-1 hour').toString();
      await serverHelper.apiClient.purgeRequests('pp/wt', { since });
      assert.equal(dbHelper.db.fns.purge_requests_wpid.callCount, 1);
      await serverHelper.apiClient.purgeRequests('pp/wt', { since });
      await serverHelper.apiClient.purgeRequests('pp/wt', { since });
      await serverHelper.apiClient.purgeRequests('pp/wt', { since });
      // not called again..
  ...
  test('purgeRequest with a failed postgres query', async function() {
    ...
    const client = serverHelper.apiClient.use({ retries: 0 });

    sinon.stub(dbHelper.db.fns, 'purge_requests_wpid')
    ...
    try {
      ...
      assert.equal(dbHelper.db.fns.purge_requests_wpid.callCount, 2);
    } finally {
