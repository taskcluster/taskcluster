...
builder.declare(
  {
    ...
    title: 'Get Client',
    description: ['Get information about a single client.'].join('\n'),
  },
  async function (req, res) {
...
builder.declare(
  {
    ...
    scopes: {
      AllOf: ['auth:create-client:<clientId>', { for: 'scope', in: 'scopes', each: '<scope>' }],
    },
    stability: 'stable',
    title: 'Create Client',
    description: [
  ...
  async function (req, res) {
    ...
    // Send pulse message
    await Promise.all([this.publisher.clientCreated({ clientId }), this.resolver.reloadClient(clientId)]);
...
builder.declare(
  ...
  async function (req, res) {
    ...
    await Promise.all([this.publisher.clientUpdated({ clientId }), this.resolver.reloadClient(clientId)]);
...
builder.declare(
  {
    ...
    output: 'get-client-response.yml',
    scopes: {
      AllOf: ['auth:update-client:<clientId>', { for: 'scope', in: 'scopesAdded', each: '<scope>' }],
    },
    stability: 'stable',
    title: 'Update Client',
    description: [
...
builder.declare(
  ...
  async function (req, res) {
    ...
    await Promise.all([this.publisher.clientDeleted({ clientId }), this.resolver.reloadClient(clientId)]);
...
builder.declare(
  {
    ...
    stability: 'stable',
    title: 'Get Role',
    description: ['Get information about a single role, including the set of scopes that the', 'role expands to.'].join(
      '\n',
    ),
  },
  async function (req, res) {
...
builder.declare(
  {
    ...
    output: 'get-role-response.yml',
    scopes: {
      AllOf: ['auth:create-role:<roleId>', { for: 'scope', in: 'scopes', each: '<scope>' }],
    },
    stability: 'stable',
    title: 'Create Role',
    description: [
  ...
  async function (req, res) {
    ...
    try {
      ...
      await this.db.fns.insert_auth_audit_history(roleId, 'role', await req.clientId(), AUDIT_ENTRY_TYPE.ROLE.CREATED);
    } catch (err) {
      switch (err.code) {
    ...
    await Promise.all([this.publisher.roleCreated({ roleId }), this.resolver.reloadRoles()]);
...
builder.declare(
  {
    ...
    output: 'get-role-response.yml',
    scopes: {
      AllOf: ['auth:update-role:<roleId>', { for: 'scope', in: 'scopesAdded', each: '<scope>' }],
    },
    stability: 'stable',
    title: 'Update Role',
    description: [
  ...
  async function (req, res) {
    ...
    try {
      ...
      await this.db.fns.insert_auth_audit_history(roleId, 'role', await req.clientId(), AUDIT_ENTRY_TYPE.ROLE.UPDATED);
    } catch (err) {
      switch (err.code) {
        case 'InvalidScopeError':
          return res.reportError('InputError', err.message, { scope: err.scope });
    ...
    // Publish message on pulse to clear caches...
    await Promise.all([this.publisher.roleUpdated({ roleId }), this.resolver.reloadRoles()]);
...
builder.declare(
  {
    ...
    stability: 'stable',
    title: 'Delete Role',
    description: ['Delete a role. This operation will succeed regardless of whether or not', 'the role exists.'].join(
      '\n',
    ),
  },
  async function (req, res) {
    ...
    await this.db.fns.insert_auth_audit_history(roleId, 'role', await req.clientId(), AUDIT_ENTRY_TYPE.ROLE.DELETED);

    await Promise.all([this.publisher.roleDeleted({ roleId }), this.resolver.reloadRoles()]);
...
builder.declare(
  {
    ...
    stability: 'stable',
    title: 'Expand Scopes',
    description: ['Return an expanded copy of the given scopeset, with scopes implied by any', 'roles included.'].join(
      '\n',
    ),
  },
  async function (req, res) {
...
builder.declare(
  ...
  async function (req, res) {
    await new Promise((next) =>
      APIBuilder.middleware.remoteAuthentication({
        ...
        entry: {
          route: '/test-authenticate',
          scopes: { AllOf: [{ for: 'scope', in: 'requiredScopes', each: '<scope>' }] },
        },
      })(req, res, next),
    );
    await req.authorize({ requiredScopes: req.body.requiredScopes || [] });
    const [clientId, scopes] = await Promise.all([req.clientId(), req.scopes()]);
