...
export const azureBuilder = (builder) => {
  builder.declare(
    {
      method: 'get',
      ...
      description: ['Retrieve a list of all Azure accounts managed by Taskcluster Auth.'].join('\n'),
    },
    function (_req, res) {
  ...
  builder.declare(
    {
      ...
      description: ['Retrieve a list of all tables in an account.'].join('\n'),
    },
    async function (req, res) {
      ...
      if (!accessKey) {
        return res.reportError('ResourceNotFound', `Account '${account}' not found, can't delegate access`);
      }
  ...
  builder.declare(
    {
      ...
      description: ['Retrieve a list of all containers in an account.'].join('\n'),
    },
    async function (req, res) {
      const account = req.params.account;
      const continuationToken = req.query.continuationToken || null;
