...
export const awsBuilder = (builder) =>
  builder.declare(
    ...
    async function (req, res) {
      ...
      const bucketCreds = _.find(awsCredentials.allowedBuckets || {}, ({ buckets }) => buckets.includes(bucket));
      if (!bucketCreds) {
        return res.reportError('ResourceNotFound', 'No credentials available for bucket {{bucket}}', { bucket });
      }
      ...
      if (prefix[0] === '/') {
        return res.reportError('InputError', 'The `prefix` may not start with a slash `/`; got `{{prefix}}`', {
          prefix,
        });
      }

      // Decide actions to be allowed on S3 objects
      const objectActions = ['s3:GetObject'];
      if (level === 'read-write') {
        objectActions.push('s3:PutObject', 's3:DeleteObject');
      }

      // For details on the policy see: http://amzn.to/1ETStaL
      const iamReq = await sts.send(
            ...
            Statement: [
              {
                ...
                Resource: [
                  'arn:aws:s3:::{{bucket}}/{{prefix}}*'.replace('{{bucket}}', bucket).replace('{{prefix}}', prefix),
                ],
              },
              {
                ...
                Resource: ['arn:aws:s3:::{{bucket}}'.replace('{{bucket}}', bucket)],
                Condition: {
                  StringLike: {
                    's3:prefix': ['{{prefix}}*'.replace('{{prefix}}', prefix)],
                  },
                },
              },
              {
