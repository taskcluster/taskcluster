...
helper.secrets.mockSuite(testing.suiteName(), ['aws'], function (mock, skipping) {
  ...
  test('createTask', async () => {
    ...
    assert.deepEqual(
      monitor.manager.messages.find(({ Type }) => Type === 'task-defined'),
      {
  ...
  test('createTask (without required scopes)', async () => {
    const taskId = slugid.v4();
    helper.scopes('queue:create-task:lowest:my-provisioner/another-worker', 'queue:route:wrong-route');
  ...
  test('createTask (with ** scope)', async () => {
    ...
    await helper.queue.createTask(taskId, _.defaults({ scopes: ['abc:**'] }, taskDef)).then(
      () => {
        throw new Error('Expected an authentication error');
      },
      (err) => {
  ...
  test('createTask is idempotent even when it fails sending pulse messages', async () => {
    ...
    try {
      try {
        await helper.queue.use({ retries: 0 }).createTask(taskId, taskDef);
      } catch (err) {
  ...
  ['ssh://git@github.com:taskcluster/taskcluster-queue', 'git@github.com:taskcluster/taskcluster-queue'].forEach(
    (source) => {
