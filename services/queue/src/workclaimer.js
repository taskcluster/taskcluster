...
class WorkClaimer extends events.EventEmitter {
      ...
      claims = await Promise.all(
        hints.map(async (hint) => {
          try {
            ...
            const result = await this._monitor.timer(
              'claimTask',
              this.claimTask(hint.taskId, hint.runId, workerGroup, workerId, null, hint.hintId),
            );
  ...
  async claimTask(taskId, runId, workerGroup, workerId, task = null, hintId = null) {
    ...
    task.updateStatusWith(await this.db.fns.claim_task(taskId, runId, workerGroup, workerId, hintId, takenUntil));
    ...
    const credentials = taskCreds(taskId, runId, workerGroup, workerId, takenUntil, task.scopes, this._credentials);
