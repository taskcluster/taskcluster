...
const load = loader(
  {
    ...
    publisher: {
      ...
      setup: async ({ cfg, schemaset, pulseClient }) =>
        exchanges.publisher({
    ...
    workerManagerEvents: {
      requires: ['cfg'],
      setup: ({ cfg }) =>
        new taskcluster.WorkerManagerEvents({
          rootUrl: cfg.taskcluster.rootUrl,
        }),
    },
    ...
    workClaimer: {
      ...
      setup: ({ cfg, publisher, db, queueService, monitor }) =>
        new WorkClaimer({
          publisher,
          db,
          queueService,
        ...
        libReferences
          .fromService({
            ...
            references: [
              builder.reference(),
              exchanges.reference(),
              MonitorManager.reference('queue'),
              MonitorManager.metricsReference('queue'),
            ],
          })
    ...
    'claim-resolver': {
      requires: ['cfg', 'db', 'queueService', 'publisher', 'monitor', 'dependencyTracker'],
      setup: async ({ cfg, db, queueService, publisher, dependencyTracker, monitor }, ownName) => {
    ...
    'worker-removed-resolver': {
      requires: ['cfg', 'db', 'publisher', 'monitor', 'dependencyTracker', 'pulseClient', 'workerManagerEvents'],
      setup: async ({ db, publisher, dependencyTracker, monitor, pulseClient, workerManagerEvents }) => {
    ...
    'expire-artifacts': {
      ...
      setup: ({ cfg, db, publicArtifactBucket, privateArtifactBucket, monitor }, ownName) => {
        return monitor.oneShot(ownName, async () => {
          ...
          debug(
            'Expiring artifacts at: %s, from before %s, useBulkDelete: %s, batchSize: %d',
            new Date(),
            now,
            useBulkDelete,
