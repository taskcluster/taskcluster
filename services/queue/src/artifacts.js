...
const getArtifactFollowingLinks = async function ({ taskId, runId, name, req, res }) {
  ...
  while (true) {
    ...
    if (names.indexOf(targetName) !== -1) {
      return res.reportError('InputError', 'Artifact leads to a link cycle', {});
    }
...
const generateS3Url = async function ({ artifact, skipCDN, req }) {
  ...
  if (this.signPublicArtifactUrls || bucket === this.privateBucket.bucket) {
    const bucketObject = bucket === this.privateBucket.bucket ? this.privateBucket : this.publicBucket;
  ...
  builder.declare(
    ...
    async function (req, res) {
      ...
      if (expires.getTime() < past.getTime()) {
        return res.reportError('InputError', 'Expires must be in the future', {});
      }
      ...
      if (!task) {
        return res.reportError('InputError', 'Task not found', {});
      }

      // Check presence of the run
      const run = task.runs[runId];
      if (!run) {
        return res.reportError('InputError', 'Run not found', {});
      }
      ...
      if (!workerGroup || !workerId) {
        return res.reportError('InputError', 'Run was not claimed by a worker and so no artifacts can exist', {});
      }
      ...
      try {
        artifact = artifactUtils.fromDbRows(
          await this.db.fns.create_queue_artifact_2(
            taskId,
            runId,
            name,
            storageType,
      ...
      switch (artifact.storageType) {
        case 'object': {
          ...
          const credentials = taskcluster.createTemporaryCredentials({
            ...
            start: new Date(),
            expiry,
            scopes: [`object:upload:${projectId}:${objectName}`],
        ...
        case 's3': {
          ...
          // Create put URL
          const putUrl = await bucket.createPutUrl(artifact.details.prefix, {
  ...
  builder.declare(
    ...
    async function (req, res) {
      ...
      if (uploadId !== artifact.details?.uploadId) {
        return res.reportError('InputError', 'given uploadId does not match that for the artifact', {});
      }
  ...
  const replyWithArtifactsList = async function ({ query, taskId, runId, res }) {
    const artifacts = await paginateResults({
      ...
      fetch: (page_size_in, after) =>
        this.db.fns.get_queue_artifacts_paginated_2({
          task_id_in: taskId,
          run_id_in: runId,
          expires_in: null,
          page_size_in,
  ...
  builder.declare(
    ...
    async function (req, res) {
      ...
      if (runId < 0 || runId > latestRunId) {
        return res.reportError('ResourceNotFound', 'Task with taskId: `{{taskId}}` has no run with runId: {{runId}}', {
          taskId,
          runId,
        });
      }
