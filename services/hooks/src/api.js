...
const builder = new APIBuilder({
  title: 'Hooks Service',
  description: ['The hooks service provides a mechanism for creating tasks in response to events.', ''].join('\n'),
  serviceName: 'hooks',
  apiVersion: 'v1',
  params: {
...
builder.declare(
  {
    ...
    stability: 'stable',
    description: ['This endpoint will return a list of all hook groups with at least one hook.'].join('\n'),
  },
...
builder.declare(
  {
    ...
    stability: 'stable',
    description: ['This endpoint will return a list of all the hook definitions within a', 'given hook group.'].join(
      '\n',
    ),
  },
...
builder.declare(
  {
    ...
    category: 'Hooks',
    stability: 'stable',
    description: ['This endpoint will return the hook definition for the given `hookGroupId`', 'and hookId.'].join(
      '\n',
    ),
  },
  async function (req, res) {
...
builder.declare(
  {
    ...
    route: '/hooks/:hookGroupId/:hookId',
    name: 'createHook',
    scopes: { AllOf: ['hooks:modify-hook:<hookGroupId>/<hookId>', 'assume:hook-id:<hookGroupId>/<hookId>'] },
  ...
  async function (req, res) {
    ...
    for (const schedElement of hookDef.schedule) {
      try {
        ...
        return res.reportError('InputError', '{{message}} in {{schedElement}}', { message: err.message, schedElement });
      }
    }
    ...
    try {
      ...
      if (!_.isEqual(hookDef, hookUtils.definition(existingHook))) {
        return res.reportError('RequestConflict', `hook \`${hookGroupId}/${hookId}\` already exists.`, {});
      }
    }
...
builder.declare(
  {
    ...
    stability: 'stable',
    category: 'Hooks',
    description: ['This endpoint will remove a hook definition.'].join('\n'),
  },
  async function (req, res) {
    const hookGroupId = req.params.hookGroupId;
    const hookId = req.params.hookId;
...
const triggerHookCommon = async function ({ req, res, hook, payload, clientId, firedBy }) {
  ...
  if (resp) {
    ...
    return res.reportError('InputError', 'While calling {{method}}: {{code}}\n\n{{message}}', {
      code: error.body.code,
      method: error.body.requestInfo.method,
      message: error.body.message,
    });
  } else {
    return res.reportError('InputError', 'While firing hook:\n\n{{error}}', { error: (error || 'unknown').toString() });
  }
};
...
builder.declare(
  ...
  async function (req, res) {
    ...
    const { continuationToken, rows: lastFires } = await paginateResults({
      query: req.query,
      fetch: (size, offset) => this.db.fns.get_last_fires_with_task_state(hookGroupId, hookId, size, offset),
    });
